...
bool Application::VerifyVehicleChecksum(const uint8_t* buf, int len) {
  ...
  uint16_t received =
      static_cast<uint16_t>(buf[payload_len]) | (static_cast<uint16_t>(buf[payload_len + 1]) << 8);
  return sum == received;
}
...
Application::Application(int argc, char** argv) : userNode(argc, argv), is_running(true) {
  ...
  vehicleThread = std::make_unique<boost::thread>(
      boost::bind(&Application::processPendingDatagramsVehicle, this));
  insThread =
...
void Application::processPendingDatagramsVehicle(void) {
  ...
  while (is_running) {
    ...
    if ((0xAA == vehicle_buf[0]) && (0x55 == vehicle_buf[1]) && (iLen == VEHICLE_INFO_LENGHT) &&
        (userNode.getReady())) {
...
void Application::vehicleSendUdp() {
  ...
  if (msgLength != VEHICLE_CMD_LENGHT) {
    ROS_ERROR("[VehicleInterface] TX frame length mismatch: %d (expected %d), dropping", msgLength,
              VEHICLE_CMD_LENGHT);
    return;
  }
