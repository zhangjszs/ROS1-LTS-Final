...
UserNode::UserNode(int argc, char** pArgv) : m_Init_argc(argc), m_pInit_argv(pArgv) {
  rosRunning = false;
  m_bExit = false;
...
void UserNode::init() {
  ...
  ros_pub_vehicle_info =
      nh.advertise<autodrive_msgs::HUAT_VehicleStatus>(MSG_TOPIC_VEHICLE_STATUS, 1);
...
void UserNode::recv_msg_vehcileCMD_callack(
    const autodrive_msgs::HUAT_VehicleCmd::ConstPtr& pVehicleCMD) {
