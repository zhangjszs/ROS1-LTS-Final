...
LineDetectionCore::LineDetectionCore(const LineDetectionParams& params) : params_(params) {
...
std::vector<Pose> LineDetectionCore::GenerateFallbackPathFromSparseCones(
    const std::vector<ConePoint>& cones) const {
...
std::pair<HoughLine, HoughLine> LineDetectionCore::SelectBoundaryLines(
  ...
  for (size_t i = 0; i < std::min(lines.size(), static_cast<size_t>(params_.max_lines_to_check));
       ++i) {
    for (size_t j = i + 1;
         j < std::min(lines.size(), static_cast<size_t>(params_.max_lines_to_check)); ++j) {
...
std::vector<Pose> LineDetectionCore::GeneratePath(const HoughLine& center_line) const {
  ...
  const double path_end =
      std::max(params_.path_start_x, std::max(params_.max_path_distance, accel_plus_brake));
...
std::vector<Pose> LineDetectionCore::ConvertToWorldCoordinates(
