...
Way::Way() : avgEdgeLen_(0.0) {
  closestToCarElem_ =
      this->path_
          .cend();  // 返回的是指向 `path_`
                    // 列表的尾部的常量迭代器。这个迭代器表示列表的结束位置，用于表示迭代范围的终点。它指向的是最后一个元素之后的位置，因此不指向任何有效的元素。
  sizeToCar_ = 0;
}
...
void Way::trimByLocal() {
  ...
  auto closestToCarElem =
      this->closestToCarElem_;  // way如果无数据，则构造函数写到了结束位置  指向距离车最近位置的边
  ...
  if (closestToCarElem != std::prev(this->path_.cend())) {
    ...
    // 删除 `path_` 中最接近 `closestToCarElem`
    // 的元素之后（包括最接近的元素本身），也就是删除靠后部分的元素。
  ...
  // 其中 `it->len`
  // 是当前迭代元素的边长。这个公式可以理解为每次新增的边长与当前平均边长的偏差除以迭代次数 `t`。
...
bool Way::closesLoop() const {
  return this->size() >= MIN_LOOP_SIZE and
         Point::distSq(this->front().midPoint(), this->back().midPoint()) <=
...
Way Way::restructureClosure() const {
  ...
  if (res.front() != res.back()) {
    ...
    for (auto it = res.path_.begin(); it != std::prev(res.path_.end(), 20);
         it++) {  // The 20 is for safety
  ...
  if (res.front() == res.back() or
      ...
      Point::dist(res.back().midPointGlobal(), res.front().midPointGlobal()) <
          SAME_MIDPOINT_DIST_THRESHOLD) {
    res.path_.pop_back();
  }
...
Tracklimits Way::getTracklimits() const {
  ...
  Point pAnt = this->empty()
                   ? Point(0, 0)
                   : Point(-50, this->front()
                                    .midPointGlobal()
                                    .y);  // This only works in a global, the -50 is arbitrary
  ...
  size_t edgeInd = 0;
  for (const Edge& e : this->path_) {
    // if (this->closesLoop() and edgeInd == this->size() - 1) break;  // Last Edge is (can be) same
    // as first => nothing to append
    ...
    // Only override when color is definitive (not NONE/ORANGE).
    {
      ...
      const bool l_has_color =
          (lt != planning_core::cone_type::NONE && lt != planning_core::cone_type::ORANGE_SMALL &&
           lt != planning_core::cone_type::ORANGE_BIG);
      const bool r_has_color =
          (rt != planning_core::cone_type::NONE && rt != planning_core::cone_type::ORANGE_SMALL &&
      ...
      if (l_has_color && r_has_color) {
        ...
        if (lt == planning_core::cone_type::BLUE)
          std::swap(left, right);
      } else if (!l_has_color && r_has_color) {
        if (rt == planning_core::cone_type::YELLOW)
          std::swap(left, right);
      }
    }
    ...
    if (res.first.empty() or
        (*left != res.first.back() and
         ...
         (!this->closesLoop() or edgeInd < this->size() - 3 or
          !Vector::pointBehind(res.first.front().pointGlobal(), left->pointGlobal(), pAntPAct)))) {
      if (*left == *firstLeft) {
    ...
    if (res.second.empty() or
        (*right != res.second.back() and
         ...
         (!this->closesLoop() or edgeInd < this->size() - 3 or
          !Vector::pointBehind(res.second.front().pointGlobal(), right->pointGlobal(),
                               pAntPAct)))) {
      if (*right == *firstRight) {
...
std::vector<geometry_msgs::Point> Way::getPathInterpolation(double x, double y) {
  ...
  if (pow(it->midPointGlobal().x - x, 2) + pow(it->midPointGlobal().y - y, 2) <
      pow(it->midPointGlobal().x - lastIt->midPointGlobal().x, 2) +
          pow(it->midPointGlobal().y - lastIt->midPointGlobal().y, 2) / 4) {
    size++;
    // 这是虚拟点的x,y值画出来就是车到第二个点的中点与第一个点到第二点中点的连线的中点
    // 其代替车前的第一个点,其相对与直接利用第一个点来说更方便拟合而且防止折角过大的出现
    double helpmidass_x = ((it->midPointGlobal().x + x) / 2 +
                           (it->midPointGlobal().x + lastIt->midPointGlobal().x) / 2) /
                          2;
    double helpmidass_y = ((it->midPointGlobal().y + y) / 2 +
                           (it->midPointGlobal().y + lastIt->midPointGlobal().y) / 2) /
                          2;
    ...
    if ((x > helpmidass_x && helpmidass_x > it->midPointGlobal().x) ||
        (x < helpmidass_x && helpmidass_x < it->midPointGlobal().x))
      start_x = true;
    if ((y < helpmidass_y && helpmidass_y < it->midPointGlobal().y) ||
        (y > helpmidass_y && helpmidass_y > it->midPointGlobal().y))
      start_y = true;
    ...
    // 拟合一个简单的3项式
    Eigen::VectorXd yn = A.colPivHouseholderQr().solve(
        b);  // 构建线性方程组的系数矩阵A，这里使用x的幂次来构建，是为了拟合一个多项式
    double A_d =
        (A_2 - A_0) /
        30.0;  // 两点之间分为30份，以方便对曲线拟合数据的输出
  ...
  if (size > 2) {
    ...
    for (int i = 0; i < size - 2; i++) {
      ...
      bool increasing_x = true, decreasing_x = true, increasing_y = true, decreasing_y = true,
           turn_y = false, bend = true, turn_y2 = false;
      ...
      for (size_t j = 0; j < 3; ++j) {
        ...
        if (it->midPointGlobal().y > lastIt->midPointGlobal().y)
          decreasing_y = false;
        if (abs(it->midPointGlobal().x - lastIt->midPointGlobal().x) * 2 <
            abs(it->midPointGlobal().y - lastIt->midPointGlobal().y))
          turn_y = true;
        if (abs(it->midPointGlobal().x - lastIt->midPointGlobal().x) >
            abs(it->midPointGlobal().y - lastIt->midPointGlobal().y))
          turn_y = false;
      ...
      if ((((increasing_x && !decreasing_x) || (!increasing_x && decreasing_x)) &&
           !(((increasing_y && !decreasing_y) || (!increasing_y && decreasing_y)) && turn_y)) ||
          turn_y2) {
        Eigen::MatrixXd A(3, 3);
        Eigen::VectorXd b(3);
        int n = 0;
        // 得到轨迹上最短两点的距离,最长的肯定用不了,还是用最短的距离合理
        limit = std::min((pow(lastIt->midPointGlobal().x - it->midPointGlobal().x, 2) +
                          pow(lastIt->midPointGlobal().y - it->midPointGlobal().y, 2)),
                         limit);
        for (int j = i; j <= i + 2; j++) {
