...
bool finish() {
  ...
  if (!inter && std::abs(wayComputer->getCarState().car_state.x) < 1 &&
      std::abs(wayComputer->getCarState().car_state.y) < 1) {
    inter = true;
    std::cout << "进入判定区域" << std::endl;
  }
  // 如果标记进入，并且x>1，那么认为开始一圈
  if (std::abs(wayComputer->getCarState().car_state.y) < 1 && inter &&
      wayComputer->getCarState().car_state.x > 1) {
    inter = false;
    interTimes++;
...
void callback_ccat(const autodrive_msgs::HUAT_ConeMap::ConstPtr& data) {
  ...
  for (const autodrive_msgs::HUAT_Cone& c : data->cone) {
    ...
    if (static_cast<double>(p.x) >= 0.0 && dist >= params->main.far_front_center_dist_threshold &&
        std::abs(static_cast<double>(p.y)) <= params->main.far_front_center_abs_y_threshold) {
      required_min = std::max(
          required_min, static_cast<double>(params->main.min_cone_confidence_far_front_center));
    }
    if (static_cast<double>(p.x) >= 0.0 && dist >= params->main.far_side_dist_threshold &&
        std::abs(static_cast<double>(p.y)) > params->main.far_side_abs_y_threshold) {
      required_min =
          std::max(required_min, static_cast<double>(params->main.min_cone_confidence_far_side));
    }
    if (static_cast<double>(p.x) >= 0.0 && dist >= params->main.far_side_dist_threshold &&
        std::abs(static_cast<double>(p.y)) > params->main.far_side_abs_y_strict_threshold) {
      required_min = std::max(
          required_min, static_cast<double>(params->main.min_cone_confidence_far_side_strict));
    }

    if (params->main.cone_geom_filter_enable) {
      const bool front_ok =
          !params->main.cone_geom_filter_front_only || static_cast<double>(p.x) >= 0.0;
      if (front_ok && dist >= params->main.cone_geom_filter_dist_th &&
  ...
  if (nodes.empty()) {
    ROS_WARN_THROTTLE(
        1.0, "[high_speed_tracking] No cones passed confidence filter (min_cone_confidence=%.3f).",
  ...
  if (loop_fallback_active_now && !loopFallbackWasActive) {
    ROS_WARN(
        "[high_speed_tracking] Loop-closure fallback activated by lap counter (interTimes=%d, "
        "threshold=%d).",
  ...
  if (loop_closed_for_publish) {
    ...
    autodrive_msgs::HUAT_PathLimits full_msg =
        wayComputer->getPathLimitsGlobal(params->main.the_mode_of_full_path);
    ...
    if (wasLoopClosed) {
      ROS_WARN_THROTTLE(
          1.0, "[high_speed_tracking] FAST_LAP lost. Fallback to SAFE_LAP partial output.");
    }
    autodrive_msgs::HUAT_PathLimits partial_msg =
        wayComputer->getPathLimitsGlobal(params->main.the_mode_of_partial_path);
    if (params->main.debug_save_way_files) {
  ...
  if (finish_reached) {
    if (waitFullBeforeStop && !fullPathPublishedOnce &&
        finishGraceCount < std::max(0, finishGraceFrames)) {
      ++finishGraceCount;
      ROS_WARN_THROTTLE(
          1.0,
