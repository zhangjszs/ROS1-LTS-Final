...
double signedCurvature(const geometry_msgs::Point& p0, const geometry_msgs::Point& p1,
...
void smoothPathForCurvature(std::vector<geometry_msgs::Point>& path, const double curvature_limit,
...
void smoothPathSpatialBlend(std::vector<geometry_msgs::Point>& path, const int half_window,
                            const int passes, const double blend) {
...
int countCurvatureViolations(const std::vector<Point>& path, const double curvature_limit) {
...
bool detectLoopClosureByHistory(const std::vector<Point>& path_local, const double dist_threshold,
                                const double angle_threshold, double& out_best_dist,
                                int& out_best_idx, double& out_best_angle) {
  out_best_dist = -1.0;
  out_best_idx = -1;
  out_best_angle = -1.0;
  ...
  const size_t front_window =
      std::min(search_end, std::max<size_t>(static_cast<size_t>(MIN_LOOP_SIZE), n / 3));
  if (front_window < 2) {
    return false;
  }
...
void WayComputer::filterMidpoints(EdgeSet& edges, const TriangleSet& triangulation) const {
  ...
  KDTree circumKDTree(
      circums);  //这句代码的意思是以`circums`中的点作为初始数据，创建一个KD树对象`circumKDTree`

  // Perform the filtering
  auto it = edges.begin();
  while (it != edges.end()) {
    Point midPoint = it->midPoint();
    KDTData<size_t> nearestCC = circumKDTree.nearest_index(
        midPoint);  //通过比较目标点与KD树中的点，我们可以找到离目标点最近的外接圆心
    //如果`nearestCC`有效（非空）并且中点与最近外接圆心的距离的平方大于
    //`this->params_.max_dist_circum_midPoint`的平方，
    //则说明中点距离外接圆心太远，需要将该边从`edges`中移除
    if (bool(nearestCC) and Point::distSq(circums[*nearestCC], midPoint) >
                                pow(this->params_.max_dist_circum_midPoint, 2)) {
      it = edges.erase(it);
    } else {
      it++;
    }
...
double WayComputer::getHeuristic(const Point& actPos, const Point& nextPos, const Vector& dir,
  ...
  double angleHeur =
      -log(std::max(0.0, ((M_PI_2 - abs(angle)) / M_PI_2) -
                             0.2));  //这个计算过程的目的是将夹角越接近0，启发式函数值越大
...
void WayComputer::findNextEdges(std::vector<HeurInd>& nextEdges, const Trace* actTrace,
  ...
  const Edge* actEdge =
      nullptr;  // Last valid Edge (starting point)最近有效的边（起始点）表示当前没有有效的边
  ...
  if (actTrace and not actTrace->empty()) {
    ...
    if (actTrace->size() >= 2) {
      lastPos =
          edges[actTrace->before().edgeInd()].midPoint();  //前一个的中点作为这个前一时刻处理。
    }
  }
  ...
  //并且当前路径 `this->way_` 的大小大于等于 2 或者存在有效的追踪信息
  //`actTrace`，则会执行条件语句中的代码块。
  Vector dir;
  if (actEdge and
      (this->way_.size() >= 2 or actTrace))  // This condition avoids setting a vector pointing
                                             // backwards  用于防止设置一个指向后方的方向向量
  ...
  //无序不重复的数据
  std::unordered_set<size_t> nextPossibleEdges =
      midpointsKDT.neighborhood_indices_set(actPos, params.search_radius);

  // Discard edges by specifications
  //根据规则筛选并丢弃边
  auto it = nextPossibleEdges.begin();
  while (it != nextPossibleEdges.end()) {
    const Edge& nextPossibleEdge =
        edges[*it];  //是在将索引集合 `nextPossibleEdges` 中的索引值 `*it` 用作 `edges`
                     //数组的索引，以获取对应的边对象。
    ...
    bool removeConditions =
        ...
        (
            ...
            // 角度过大的边就删除
            abs(dir.angleWith(Vector(actPos, nextPossibleEdge.midPoint()))) >
                params.max_angle_diff or

            // 3. [Only before appending the edge that closes the loop] Remove any edge that is
            // already contained in the path but is not the one that closes the loop
            // 它的作用是在路径尚未闭合之前，从候选边集合中移除任何已经存在于路径中但并不是用来闭合路径的边。
            (not this->way_.closesLoopWith(nextPossibleEdge) and
             (not actTrace or not actTrace->isLoopClosed()) and
             this->way_.containsEdge(nextPossibleEdge)) or

            // 4. Remove any edge whose midpoint and lastPos are in the same side of actEdge (avoid
            // bouncing on a track limit)
            //该条件的目的是移除处于同一侧的候选边，以避免在路径上来回反弹。通过移除这样的边，可以确保路径的平滑性和连续性
            ((this->way_.size() >= 2 or actTrace) and
             Vector::pointBehind(actEdge->midPoint(), lastPos, actEdge->normal()) ==
                 Vector::pointBehind(actEdge->midPoint(), nextPossibleEdge.midPoint(),
                                     actEdge->normal())) or

            // 5. Remove any edge whose length is too big or too small compared to the average edge
            // length of the way
            //该条件的目的是过滤掉边长与路径平均边长相差过大的边，以确保路径的边长变化合理。通过移除过大或过小的边长，
            //可以防止路径在边长较大或较小的区域上出现不连续或不平滑的变化。
            (nextPossibleEdge.len <
                 (1 - params.edge_len_diff_factor) * this->avgEdgeLen(actTrace) or
             nextPossibleEdge.len >
                 (1 + params.edge_len_diff_factor) * this->avgEdgeLen(actTrace)) or

            // 6. [If not allow_intersection, only before closing the loop] Remove any Edge which
            // appended would create an intersection
            (not params.allow_intersection and (not actTrace or not actTrace->isLoopClosed()) and
             not this->way_.closesLoopWith(nextPossibleEdge) and
             this->way_.intersectsWith(nextPossibleEdge)));
  ...
  for (const size_t& nextPossibleEdgeInd : nextPossibleEdges) {
    double heuristic =
        this->getHeuristic(actPos, edges[nextPossibleEdgeInd].midPoint(), dir, params);
  ...
  std::partial_sort_copy(privilege_runner.begin(), privilege_runner.end(), nextEdges.begin(),
                         nextEdges.end());
}
...
Trace WayComputer::computeBestTraceWithFinishedT(const Trace& best, const Trace& t) const {
  ...
  // 2. 如果 `t` 的长度与 `best` 的长度相等，并且 `t` 的累积启发值（accum heuristic）小于 `best`
  // 的累积启发值，则 `t` 是更优的路径。
...
size_t WayComputer::treeSearch(std::vector<HeurInd>& nextEdges, const KDTree& midpointsKDT,
  ...
  // 2.
  // 当从树搜索开始时间到当前时间的耗时超过了`params.max_treeSearch_time`时限，表示超过了设定的时间限制。
...
void WayComputer::computeWay(const std::vector<Edge>& edges,
  ...
  // Trace
  // 类的作用是提供了一个用于记录和操作路径追踪信息的通用工具，使其更方便地用于路径规划等应用场景。
...
void WayComputer::fillPathDynamics(autodrive_msgs::HUAT_PathLimits& msg) const {
  ...
  if (speed_cfg.curvature_smoothing_enable && n >= 3) {
    smoothPathForCurvature(msg.path, kappa_limit, speed_cfg.curvature_smoothing_max_iters,
  ...
  const bool severe_violation = (kappa_violations >= warn_min_points) &&
  ...
  auto try_extra_smoothing = [&](double limit_scale, int extra_iters, double alpha) {
    ...
    if (new_violations <= prev_violations) {
      if (new_violations < prev_violations) {
        ROS_INFO_THROTTLE(
            1.0, "[high_speed_tracking] Extra curvature smoothing improved violations %d -> %d.",
  ...
  auto try_spatial_blend_smoothing = [&](int half_window, int passes, double blend) {
    ...
    if (new_violations <= prev_violations) {
      if (new_violations < prev_violations) {
        ROS_INFO_THROTTLE(1.0,
                          "[high_speed_tracking] Spatial curvature smoothing improved violations "
                          "%d -> %d (window=%d passes=%d).",
  ...
  if (speed_cfg.curvature_smoothing_enable && n >= 3) {
    const bool still_warn = (kappa_violations >= warn_min_points) &&
    ...
    if (still_warn) {
      ...
      const bool still_bad = (kappa_violations >= warn_min_points) &&
                             (violation_ratio >= std::max(0.05, warn_ratio * 0.8));
      if (still_bad) {
      ...
      const bool still_severe = (kappa_violations >= warn_min_points) &&
                                (violation_ratio >= std::max(0.05, warn_ratio * 0.8));
      if (still_severe) {
        try_spatial_blend_smoothing(2, 2, 0.45);
      }
      const bool still_high = (kappa_violations >= warn_min_points) &&
                              (violation_ratio >= std::max(0.05, warn_ratio * 0.8));
      if (still_high) {
  ...
  if (kappa_violations >= warn_min_points && violation_ratio >= warn_ratio) {
    ROS_WARN_THROTTLE(1.0,
                      "[high_speed_tracking] Curvature exceeds warning limit %.3f (base %.3f x "
                      "%.2f) at %d/%zu points (ratio=%.3f).",
                      kappa_warn_limit, kappa_limit, warn_limit_scale, kappa_violations, n,
                      violation_ratio);
  }
...
void WayComputer::updateLapMode(bool loop_closed_now) {
  ...
  if (this->lapMode_ == LapMode::MAP_BUILD_SAFE) {
    ...
    if (allow_reopen && this->modeHoldFrames_ == 0 && this->loopOpenDebounceCount_ >= open_frames) {
  ...
  if (prev_mode != this->lapMode_) {
    ...
    ROS_INFO_STREAM("[high_speed_tracking] Lap mode switched to "
                    << new_mode << " (loop_closed_now=" << (loop_closed_now ? "true" : "false")
                    << ")");
  }
}
...
WayComputer::WayComputer(const Params::WayComputer& params) : params_(params), currentStamp_(0) {
  Way::init(params.way);
  this->generalFailsafe_.initGeneral(this->params_.search,
                                     this->params_.general_failsafe_safetyFactor,
...
void WayComputer::update(TriangleSet& triangulation, const ros::Time& stamp) {
  ...
  //     And update stamp.
  //根据代码，`this->way_` 可以认为是一个不断累加的全局路径，而 `this->lastWay_`
  //用于存储上一次的全局路径
  this->lastWay_ = this->way_;  // Way构造函数直接赋值  表示路径
  this->lastStamp_ = stamp;
  this->currentStamp_ = stamp;

  // #1: Remove all triangles which we know will not be part of the
  // track.用于从三角测量中移除那些不会成为轨迹的三角形
  ...
  auto compute_adaptive_level = [&](const int short_streak, const bool force_retry) -> int {
    if (!speed_cfg.search_adaptive_enable)
      return 0;
    ...
    if (max_level == 0)
      return 0;
    if (short_streak < trigger_frames && !force_retry)
      return 0;
  ...
  auto apply_adaptive_search = [&](Params::WayComputer::Search& search_cfg, const int level) {
    ...
    search_cfg.search_radius = std::min(radius_limit, this->params_.search.search_radius +
                                                          radius_step * static_cast<double>(level));
    search_cfg.max_next_heuristic =
        std::min(heuristic_limit, this->params_.search.max_next_heuristic +
                                      heuristic_step * static_cast<double>(level));
  };
  ...
  auto adaptive_min_publish_threshold = [&](const int level) -> size_t {
    ...
    const size_t floor = static_cast<size_t>(
        std::max<int>(static_cast<int>(MIN_FAILSAFE_WAY_SIZE) + 1, scaled_floor));
  ...
  auto stable_short_path_threshold = [&](const size_t threshold) -> size_t {
    ...
    if (this->hasStableWay_) {
      const size_t relax_pts =
          static_cast<size_t>(std::max(0, speed_cfg.short_path_stable_relax_points));
      if (relax_pts > 0) {
    ...
    return std::max(relaxed, static_cast<size_t>(
                                 std::max<int>(1, static_cast<int>(MIN_FAILSAFE_WAY_SIZE) + 1)));
  };
  ...
  if (speed_cfg.search_adaptive_enable && !this->hasStableWay_) {
    const int bootstrap_level = std::clamp(speed_cfg.search_adaptive_bootstrap_level, 0,
  ...
  if (adaptive_level != this->adaptiveSearchLevel_) {
    ROS_INFO_STREAM_THROTTLE(1.0, "[high_speed_tracking] Adaptive search level "
  ...
  if (short_path && speed_cfg.search_adaptive_enable && speed_cfg.search_adaptive_retry_on_short &&
      !this->loopClosedRaw_) {
    int retry_level = compute_adaptive_level(this->shortPathStreak_, true);
    if (speed_cfg.search_adaptive_enable && !this->hasStableWay_) {
      const int bootstrap_retry = std::clamp(speed_cfg.search_adaptive_bootstrap_level + 1, 0,
                                             std::max(0, speed_cfg.search_adaptive_max_level));
    ...
    if (retry_level > adaptive_level) {
      ...
      if (!short_path) {
        ROS_INFO_THROTTLE(
            1.0, "[high_speed_tracking] Short-path recovered by adaptive retry (level=%d).",
            retry_level);
      }
    }
  }
  ...
  if (ahead_path_short && speed_cfg.search_adaptive_enable && !this->loopClosedRaw_) {
    const int rescue_level = std::clamp(
        std::max(adaptive_level + 1, compute_adaptive_level(this->shortPathStreak_, true)), 0,
        std::max(0, speed_cfg.search_adaptive_max_level));
    if (rescue_level > adaptive_level) {
      ...
      if (!ahead_path_short) {
        ROS_INFO_THROTTLE(
            1.0, "[high_speed_tracking] Ahead-path recovered by adaptive rescue (level=%d).",
            rescue_level);
      }
    }
  }

  const bool can_hold_now =
      speed_cfg.hold_last_valid_path && this->hasStableWay_ && this->holdFramesRemaining_ > 0;
  if (this->params_.general_failsafe && !this->loopClosedRaw_ && ahead_path_short &&
      !can_hold_now) {
    ROS_WARN_THROTTLE(
        1.0, "[high_speed_tracking] GENERAL FAILSAFE ACTIVATED (ahead=%u, publish=%zu).",
        static_cast<unsigned int>(this->way_.sizeAheadOfCar()), this->wayToPublish_.size());
  ...
  if (!this->loopClosedRaw_ && this->hasStableWay_ && way_local_for_loop.size() < MIN_LOOP_SIZE) {
    if (this->lastStableWay_.closesLoop()) {
      this->loopClosedRaw_ = true;
      loop_closed_strict = true;
      loop_closed_stable_source = true;
      ROS_INFO_THROTTLE(
          1.0, "[high_speed_tracking] Geometric loop closure by stable-path strict check.");
    } else {
  ...
  if (!this->loopClosedRaw_) {
    ...
    loop_closed_history =
        detectLoopClosureByHistory(way_local_for_loop, hist_dist_threshold, hist_angle_threshold,
                                   loop_hist_dist, loop_hist_idx, loop_hist_angle);
    if (loop_closed_history) {
      this->loopClosedRaw_ = true;
      ROS_INFO_THROTTLE(1.0,
                        "[high_speed_tracking] Geometric loop closure by %s history candidate "
                        "(dist=%.3f idx=%d angle=%.3f).",
                        loop_closed_stable_source ? "stable-path" : "current-path", loop_hist_dist,
                        loop_hist_idx, loop_hist_angle);
    }
  }
  ...
  if (speed_cfg.loop_diag_enable) {
    ...
    ROS_INFO_THROTTLE(1.0,
                      "[high_speed_tracking] loop_diag raw=%d strict=%d hist=%d stable_src=%d "
                      "way_size=%zu dist_front_back=%.3f dist_back_hist_min=%.3f min_hist_idx=%d "
                      "hist_dist=%.3f hist_idx=%d hist_angle=%.3f",
                      this->loopClosedRaw_ ? 1 : 0, loop_closed_strict ? 1 : 0,
                      loop_closed_history ? 1 : 0, loop_closed_stable_source ? 1 : 0, wn,
                      dist_front_back, dist_back_hist_min, min_hist_idx, loop_hist_dist,
                      loop_hist_idx, loop_hist_angle);
  }
  ...
  const int kappa_violations = countCurvatureViolations(quality_path, quality_curvature_limit);
  const double kappa_ratio = quality_path.empty() ? 0.0
                                                  : static_cast<double>(kappa_violations) /
                                                        static_cast<double>(quality_path.size());
  const bool bad_curvature_quality =
      !short_path && (kappa_violations >= std::max(1, speed_cfg.curvature_warn_min_points)) &&
  ...
  if (!short_path && !bad_curvature_quality) {
    ...
    if (bad_curvature_quality) {
      ROS_WARN_THROTTLE(1.0,
                        "[high_speed_tracking] Holding last stable path (curvature quality: %d/%zu "
                        "> %.3f, remaining_hold=%d).",
                        kappa_violations, quality_path.size(),
                        std::max(0.0, speed_cfg.curvature_warn_ratio), this->holdFramesRemaining_);
    } else {
      ROS_WARN_THROTTLE(1.0,
                        "[high_speed_tracking] Holding last stable path (short path: %zu < %zu, "
                        "remaining_hold=%d).",
    ...
    if (bad_curvature_quality) {
      ROS_WARN_THROTTLE(1.0,
                        "[high_speed_tracking] Curvature quality degraded without hold fallback "
                        "(%d/%zu exceeds ratio %.3f).",
                        kappa_violations, quality_path.size(),
                        std::max(0.0, speed_cfg.curvature_warn_ratio));
    } else {
      ROS_WARN_THROTTLE(1.0, "[high_speed_tracking] Short path without hold fallback (%zu < %zu).",
...
autodrive_msgs::HUAT_PathLimits WayComputer::getPathLimits() const {
  ...
  planning_ros::contract::FinalizePathLimitsMessage(res, this->lastStamp_,
                                                    autodrive_msgs::frame_contract::kWorld);
  ...
  for (const Node& n : tracklimits.first) {
    ...
    if (this->params_.tracklimits_filter_enable) {
      ...
      if (front_ok && dist >= this->params_.tracklimits_filter_dist_th &&
...
autodrive_msgs::HUAT_PathLimits WayComputer::getPathLimitsGlobal(int x) {
  autodrive_msgs::HUAT_PathLimits res;
  planning_ros::contract::FinalizePathLimitsMessage(res, this->currentStamp_,
                                                    autodrive_msgs::frame_contract::kWorld);
  ...
  switch (x) {
    ...
    case 4:  // 局部路径插值(局部坐标)接口尚未实现，兼容回退到2
      ROS_WARN_THROTTLE(1.0,
                        "[high_speed_tracking] mode=4 is not implemented. Fallback to mode=2.");
      res.path = this->wayToPublish_.getPathInterpolation(pose.position.x, pose.position.y);
      break;
    case 5:  // 全路径插值(局部坐标)接口尚未实现，兼容回退到3
      ROS_WARN_THROTTLE(1.0,
                        "[high_speed_tracking] mode=5 is not implemented. Fallback to mode=3.");
      res.path = this->wayToPublish_.getPathFullInterpolation();
      break;
    case 6:
