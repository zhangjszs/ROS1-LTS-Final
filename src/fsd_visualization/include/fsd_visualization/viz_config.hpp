...
inline std::array<float, 4> getConeColor(ConeType type) {
  switch (type) {
    case ConeType::BLUE:
      return CONE_BLUE;
    case ConeType::YELLOW_SMALL:
      return CONE_YELLOW_SMALL;
    case ConeType::YELLOW_BIG:
      return CONE_YELLOW_BIG;
    case ConeType::RED:
      return CONE_RED;
    default:
      return CONE_UNKNOWN;
...
const std::string MESH_CONE_ORANGE_BIG =
    "package://fsd_visualization/meshes/cone/cone_orange_big.dae";

// --- Gazebo construction_cone（带纹理贴图的高质量锥桶） ---
const std::string MESH_CONE_GAZEBO =
    "package://fsd_visualization/meshes/cone/construction_cone.dae";
...
inline std::string getConeMeshURI(ConeType type) {
  switch (type) {
    case ConeType::BLUE:
      return MESH_CONE_BLUE;
    case ConeType::YELLOW_SMALL:
      return MESH_CONE_YELLOW;
    case ConeType::YELLOW_BIG:
      return MESH_CONE_YELLOW;  // 大黄色使用黄色模型（通过scale区分大小）
    case ConeType::RED:
      return MESH_CONE_ORANGE;  // 红色使用橙色模型（红色模型暂缺）
    default:
      return MESH_CONE_YELLOW;  // 未知类型回退到黄色
  }
}
