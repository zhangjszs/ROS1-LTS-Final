...
class ConeVisualizer {
 ...
 private:
  ...
  visualization_msgs::Marker createConeMarker(double x, double y, double z, int id, int type,
                                              const std::string& ns, const std::string& frame_id,
                                              float confidence = 1.0f, bool force_white = false);

  visualization_msgs::Marker createBoundingBoxMarker(const geometry_msgs::Point32& min_pt,
                                                     const geometry_msgs::Point32& max_pt, int id,
                                                     const std::string& ns,
                                                     const std::string& frame_id);

  visualization_msgs::Marker createDistanceLabel(double x, double y, double z, float distance,
