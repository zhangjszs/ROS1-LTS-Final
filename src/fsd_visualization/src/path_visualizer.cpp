...
PathVisualizer::PathVisualizer(ros::NodeHandle& nh, ros::NodeHandle& pnh) {
  ...
  pnh.param<std::string>("topics/path_full", full_path_topic_,
  ...
  // 订阅
  sub_path_unified_ =
      nh.subscribe(unified_path_topic_, 1, &PathVisualizer::pathLimitsUnifiedCallback, this);
  if (enable_legacy_partial_full_) {
    sub_path_partial_ =
        nh.subscribe(partial_path_topic_, 1, &PathVisualizer::pathLimitsPartialCallback, this);
    sub_path_full_ =
        nh.subscribe(full_path_topic_, 1, &PathVisualizer::pathLimitsFullCallback, this);
    ROS_WARN("[PathVisualizer] Legacy partial/full compatibility mode enabled.");
  }
  sub_nav_path_ = nh.subscribe(nav_path_topic_, 1, &PathVisualizer::navPathCallback, this);
...
visualization_msgs::MarkerArray PathVisualizer::createPathMarkers(
    const autodrive_msgs::HUAT_PathLimits& msg, const std::string& ns,
