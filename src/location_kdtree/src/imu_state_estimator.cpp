...
bool ImuStateEstimator::Process(const common_msgs::HUAT_ASENSING &msg,
                                const ros::Time &stamp,
                                common_msgs::HUAT_Carstate *out)
{
  if (!initialized_)
  {
    Initialize(msg, stamp);
    if (out)
    {
...
void ImuStateEstimator::Initialize(const common_msgs::HUAT_ASENSING &msg,
                                   const ros::Time &stamp)
{
...
void ImuStateEstimator::Predict(double dt, double ax, double ay, double gyro_z)
{
  ...
  x_(0) = x_(0) + x_(2) * dt + 0.5 * a_world_x * dt * dt;
  x_(1) = x_(1) + x_(3) * dt + 0.5 * a_world_y * dt * dt;
  x_(2) = x_(2) + a_world_x * dt;
  x_(3) = x_(3) + a_world_y * dt;
  x_(4) = NormalizeAngle(x_(4) + gyro_z * dt);

  const double dawx_dyaw = -sin_yaw * ax - cos_yaw * ay;
  const double dawy_dyaw = cos_yaw * ax - sin_yaw * ay;

  Eigen::Matrix<double, 5, 5> F = Eigen::Matrix<double, 5, 5>::Identity();
  F(0, 2) = dt;
  F(1, 3) = dt;
  F(0, 4) = 0.5 * dt * dt * dawx_dyaw;
  F(1, 4) = 0.5 * dt * dt * dawy_dyaw;
  F(2, 4) = dt * dawx_dyaw;
  F(3, 4) = dt * dawy_dyaw;

  const double var_a = params_.accel_noise * params_.accel_noise;
  const double var_g = params_.gyro_noise * params_.gyro_noise;
  const double dt2 = dt * dt;
  const double dt3 = dt2 * dt;
  const double dt4 = dt2 * dt2;

  Eigen::Matrix<double, 5, 5> Q = Eigen::Matrix<double, 5, 5>::Zero();
  Q(0, 0) = 0.25 * dt4 * var_a;
  Q(1, 1) = 0.25 * dt4 * var_a;
  Q(2, 2) = dt2 * var_a;
  Q(3, 3) = dt2 * var_a;
  Q(4, 4) = dt2 * var_g;
  Q(0, 2) = 0.5 * dt3 * var_a;
                               ...
                               int yaw_index)
{
  ...
  if (yaw_index >= 0 && yaw_index < y.size())
  {
                                      ...
                                      double &east, double &north, double &up) const
{
  ...
  north = (-cos_lon0 * xd - sin_lon0 * yd) * sin_lat0 + cos_lat0 * zd;
...
{
  x = cos_origin_ * east - sin_origin_ * north;
  y = sin_origin_ * east + cos_origin_ * north;
}
}  // namespace state_estimation
