...
AnomalyStateMachine::AnomalyStateMachine(const AnomalyConfig& cfg) : cfg_(cfg) {}
...
void AnomalyStateMachine::Evaluate(double chi2_normalized, double match_ratio,
  ...
  if (match_ratio < cfg_.match_ratio_lost) {
    if (low_match_start_ < 0.0)
      low_match_start_ = timestamp;
  } else {
    low_match_start_ = -1.0;
  }
  ...
  switch (state_) {
    ...
    case AnomalyState::RELOC_A:
      ...
      else if (reloc_start_time_ > 0.0 && (timestamp - reloc_start_time_) > cfg_.reloc_a_timeout) {
    ...
    case AnomalyState::RELOC_B:
      ...
      else if (reloc_start_time_ > 0.0 && (timestamp - reloc_start_time_) > cfg_.reloc_b_timeout) {
        state_ = AnomalyState::LOST;
        reloc_start_time_ = -1.0;
      }
      break;
