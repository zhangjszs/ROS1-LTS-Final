...
class TestVisionInjectDefault(unittest.TestCase):
    def test_vision_inject_enabled_by_default(self):
        ...
        config_file = os.path.join(
            RosPack().get_path("perception_ros"), "config", "lidar_base.yaml"
        )
        ...
        self.assertGreater(
            vision_inject_config.get("min_confidence", 0), 0, "min_confidence should be set"
        )
        self.assertGreater(
            vision_inject_config.get("max_age_sec", 0), 0, "max_age_sec should be set"
        )
        self.assertAlmostEqual(
            vision_inject_config.get("max_age_sec", 0.0),
            0.15,
            places=6,
    ...
    def test_launch_file_enables_vision_inject(self):
        ...
        launch_file = os.path.join(
            RosPack().get_path("fsd_launch"), "launch", "subsystems", "perception.launch"
        )
        ...
        self.assertFalse(
            "vision_inject/enabled: false" in content.lower(),
            "Launch file should not disable vision_inject by default",
        )
