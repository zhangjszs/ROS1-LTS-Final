...
DistortionCompensatorV2::DistortionCompensatorV2(ros::NodeHandle& nh,
  ...
  if (config_.point_time_source == "auto") {
    comp_config.point_time_source =
        lidar_distortion::DistortionConfigV2::PointTimeSource::AUTO_DETECT;
  } else if (config_.point_time_source == "point_time") {
    comp_config.point_time_source =
        lidar_distortion::DistortionConfigV2::PointTimeSource::USE_POINT_TIME;
  } else if (config_.point_time_source == "angle") {
    comp_config.point_time_source =
        lidar_distortion::DistortionConfigV2::PointTimeSource::COMPUTE_FROM_ANGLE;
  }
  ...
  if (config_.extrinsics_enable) {
    comp_config.extrinsics.translation << config_.extrinsics_tx, config_.extrinsics_ty,
        config_.extrinsics_tz;
    comp_config.extrinsics.setRotationFromEuler(config_.extrinsics_roll, config_.extrinsics_pitch,
...
void DistortionCompensatorV2::imuCallback(const autodrive_msgs::HUAT_InsP2::ConstPtr& msg) {
  ...
  R_ned2body << ch * cp, sh * cp, -sp, ch * sp * sr - sh * cr, sh * sp * sr + ch * cr, cp * sr,
...
bool DistortionCompensatorV2::Compensate(CloudPtr& cloud, double timestamp) {
  ...
  if (config_.publish_debug_info && debug_pub_.getNumSubscribers() > 0) {
    ...
    ss << "Distortion V2 Debug:\n"
       ...
       << "  Velocity: [" << debug.ref_velocity.x() << ", " << debug.ref_velocity.y() << ", "
       << debug.ref_velocity.z() << "] m/s\n"
...
bool DistortionCompensatorV2::CompensateFromMsg(const sensor_msgs::PointCloud2& msg,
  ...
  if (config_.point_time_source == "point_time") {
    ...
    if (has_time) {
      ...
      ROS_WARN_THROTTLE(
          5.0, "point_time_source='point_time' but cloud has no time field, falling back to angle");
    }
  } else if (config_.point_time_source == "angle") {
    // 用户明确要求使用角度计算
  ...
  if (use_time_field) {
    ...
    return compensator_.CompensateWithTime<lidar_distortion::PointXYZIRT>(cloud_with_time, cloud,
...
bool DistortionCompensatorV2::GetLatestEgoVelocity(double& vx, double& vy, double& yaw_rate) const {
