...
auto ConeDetectionAdapter::ToSteadyDuration(const ros::Duration &duration) -> SteadyClock::duration
{
...
{
  return raw.points.size() == fused.points.size() && fused.fused_color_types.size() >= raw.points.size();
}
...
{
  stats_.pending_cache_size_max = std::max(
      stats_.pending_cache_size_max, pending_raw_.size() + pending_fused_.size());
}
...
std::vector<ConeDetectionAdapter::PublishOutput> ConeDetectionAdapter::Flush(const SteadyTimePoint &now)
{
  PruneFinalized(now);
  std::vector<PublishOutput> outputs;
...
void ConeDetectionAdapter::TrimCaches()
{
  while (pending_raw_.size() + pending_fused_.size() > config_.max_pending)
  {
    ...
    {
      oldest_fused = std::min_element(
          pending_fused_.begin(), pending_fused_.end(),
          [](const auto &lhs, const auto &rhs) {
            return lhs.second.received_at < rhs.second.received_at;
          });
    }
