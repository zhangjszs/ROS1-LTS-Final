...
bool IMUSubscriber::SyncData(std::deque<IMUData>& UnsyncedData, IMUData& synced_data,
                             double sync_time) {
  // 至少需要 1 条数据
  ...
  // 速度线性插值
  synced_data.velocity.vn =
      front_data.velocity.vn * front_scale + back_data.velocity.vn * back_scale;
  synced_data.velocity.ve =
      front_data.velocity.ve * front_scale + back_data.velocity.ve * back_scale;
  synced_data.velocity.vd =
      front_data.velocity.vd * front_scale + back_data.velocity.vd * back_scale;

  // 加速度线性插值
  synced_data.acceleration.ax =
      front_data.acceleration.ax * front_scale + back_data.acceleration.ax * back_scale;
  synced_data.acceleration.ay =
      front_data.acceleration.ay * front_scale + back_data.acceleration.ay * back_scale;
  synced_data.acceleration.az =
      front_data.acceleration.az * front_scale + back_data.acceleration.az * back_scale;

  // 角速度线性插值
  synced_data.angular_velocity.wx =
      front_data.angular_velocity.wx * front_scale + back_data.angular_velocity.wx * back_scale;
  synced_data.angular_velocity.wy =
      front_data.angular_velocity.wy * front_scale + back_data.angular_velocity.wy * back_scale;
  synced_data.angular_velocity.wz =
      front_data.angular_velocity.wz * front_scale + back_data.angular_velocity.wz * back_scale;

  // 姿态角线性插值
  synced_data.orientation.roll =
      front_data.orientation.roll * front_scale + back_data.orientation.roll * back_scale;
  synced_data.orientation.pitch =
      front_data.orientation.pitch * front_scale + back_data.orientation.pitch * back_scale;
  synced_data.orientation.heading =
      front_data.orientation.heading * front_scale + back_data.orientation.heading * back_scale;
