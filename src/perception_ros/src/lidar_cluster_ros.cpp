...
void LidarClusterRos::loadParams()
{
  ...
  {
    ROS_DEBUG_STREAM("Did not load topics/points/passthrough. Standard value is: " << passthrough_topic_);
  }
  if (!private_nh_.param<std::string>("topics/points/no_ground", no_ground_topic_, "points/no_ground"))
  {
    ROS_DEBUG_STREAM("Did not load topics/points/no_ground. Standard value is: " << no_ground_topic_);
  }
  ...
  {
    ROS_DEBUG_STREAM("Did not load use_point_cloud_pool. Standard value is: " << use_point_cloud_pool_);
  }
  ...
  {
    ROS_DEBUG_STREAM("Did not load ransac/num_iter. Standard value is: " << config_.ransac.num_iter);
  }
  ...
  {
    ROS_DEBUG_STREAM("Did not load ransac/th_seeds. Standard value is: " << config_.ransac.th_seeds);
  }
  ...
  {
    ROS_DEBUG_STREAM("Did not load force_fgs_fast_path. Standard value is: " << force_fgs_fast_path_);
  }
  ...
  {
    ROS_DEBUG_STREAM("Did not load ground_watchdog/enable. Standard value is: " << ground_watchdog_enable_);
  }
  if (!private_nh_.param<double>("ground_watchdog/warn_ms", ground_watchdog_warn_ms_, 8.0))
  {
    ROS_DEBUG_STREAM("Did not load ground_watchdog/warn_ms. Standard value is: " << ground_watchdog_warn_ms_);
  }
  ...
  {
    ROS_DEBUG_STREAM("Did not load roi/use_point_clip. Standard value is: " << config_.roi.use_point_clip);
  }
  ...
  {
    ROS_DEBUG_STREAM("Did not load roi/skidpad/x_min. Standard value is: " << config_.roi.skidpad.x_min);
  }
  if (!private_nh_.param<double>("roi/skidpad/x_max", config_.roi.skidpad.x_max, 10.0))
  {
    ROS_DEBUG_STREAM("Did not load roi/skidpad/x_max. Standard value is: " << config_.roi.skidpad.x_max);
  }
  if (!private_nh_.param<double>("roi/skidpad/y_min", config_.roi.skidpad.y_min, -3.0))
  {
    ROS_DEBUG_STREAM("Did not load roi/skidpad/y_min. Standard value is: " << config_.roi.skidpad.y_min);
  }
  if (!private_nh_.param<double>("roi/skidpad/y_max", config_.roi.skidpad.y_max, 3.0))
  {
    ROS_DEBUG_STREAM("Did not load roi/skidpad/y_max. Standard value is: " << config_.roi.skidpad.y_max);
  }
  if (!private_nh_.param<double>("roi/accel/x_min", config_.roi.accel.x_min, 0.0))
  {
    ROS_DEBUG_STREAM("Did not load roi/accel/x_min. Standard value is: " << config_.roi.accel.x_min);
  }
  if (!private_nh_.param<double>("roi/accel/x_max", config_.roi.accel.x_max, 100.0))
  {
    ROS_DEBUG_STREAM("Did not load roi/accel/x_max. Standard value is: " << config_.roi.accel.x_max);
  }
  if (!private_nh_.param<double>("roi/accel/y_min", config_.roi.accel.y_min, -3.0))
  {
    ROS_DEBUG_STREAM("Did not load roi/accel/y_min. Standard value is: " << config_.roi.accel.y_min);
  }
  if (!private_nh_.param<double>("roi/accel/y_max", config_.roi.accel.y_max, 3.0))
  {
    ROS_DEBUG_STREAM("Did not load roi/accel/y_max. Standard value is: " << config_.roi.accel.y_max);
  }
  if (!private_nh_.param<double>("roi/track/x_min", config_.roi.track.x_min, 0.0))
  {
    ROS_DEBUG_STREAM("Did not load roi/track/x_min. Standard value is: " << config_.roi.track.x_min);
  }
  if (!private_nh_.param<double>("roi/track/x_max", config_.roi.track.x_max, 20.0))
  {
    ROS_DEBUG_STREAM("Did not load roi/track/x_max. Standard value is: " << config_.roi.track.x_max);
  }
  if (!private_nh_.param<double>("roi/track/y_min", config_.roi.track.y_min, -3.0))
  {
    ROS_DEBUG_STREAM("Did not load roi/track/y_min. Standard value is: " << config_.roi.track.y_min);
  }
  if (!private_nh_.param<double>("roi/track/y_max", config_.roi.track.y_max, 3.0))
  {
    ROS_DEBUG_STREAM("Did not load roi/track/y_max. Standard value is: " << config_.roi.track.y_max);
  }
  if (!private_nh_.param<double>("roi/custom/x_min", config_.roi.custom.x_min, 0.0))
  {
    ROS_DEBUG_STREAM("Did not load roi/custom/x_min. Standard value is: " << config_.roi.custom.x_min);
  }
  if (!private_nh_.param<double>("roi/custom/x_max", config_.roi.custom.x_max, 20.0))
  {
    ROS_DEBUG_STREAM("Did not load roi/custom/x_max. Standard value is: " << config_.roi.custom.x_max);
  }
  if (!private_nh_.param<double>("roi/custom/y_min", config_.roi.custom.y_min, -3.0))
  {
    ROS_DEBUG_STREAM("Did not load roi/custom/y_min. Standard value is: " << config_.roi.custom.y_min);
  }
  if (!private_nh_.param<double>("roi/custom/y_max", config_.roi.custom.y_max, 3.0))
  {
    ROS_DEBUG_STREAM("Did not load roi/custom/y_max. Standard value is: " << config_.roi.custom.y_max);
  }
  if (!private_nh_.param<bool>("filters/sor/enable", config_.filters.sor.enable, false))
  {
    ROS_DEBUG_STREAM("Did not load filters/sor/enable. Standard value is: " << config_.filters.sor.enable);
  }
  if (!private_nh_.param<int>("filters/sor/mean_k", config_.filters.sor.mean_k, 50))
  {
    ROS_DEBUG_STREAM("Did not load filters/sor/mean_k. Standard value is: " << config_.filters.sor.mean_k);
  }
  if (!private_nh_.param<double>("filters/sor/stddev_mul", config_.filters.sor.stddev_mul, 1.0))
  {
    ROS_DEBUG_STREAM("Did not load filters/sor/stddev_mul. Standard value is: " << config_.filters.sor.stddev_mul);
  }
  if (!private_nh_.param<bool>("filters/voxel/enable", config_.filters.voxel.enable, true))
  {
    ROS_DEBUG_STREAM("Did not load filters/voxel/enable. Standard value is: " << config_.filters.voxel.enable);
  }
  if (!private_nh_.param<double>("filters/voxel/leaf_size", config_.filters.voxel.leaf_size, 0.05))
  {
    ROS_DEBUG_STREAM("Did not load filters/voxel/leaf_size. Standard value is: " << config_.filters.voxel.leaf_size);
  }
  if (!private_nh_.param<bool>("filters/adaptive_voxel/enable", config_.filters.adaptive_voxel.enable, false))
  {
    ROS_DEBUG_STREAM("Did not load filters/adaptive_voxel/enable. Standard value is: " << config_.filters.adaptive_voxel.enable);
  }
  if (!private_nh_.param<double>("filters/adaptive_voxel/leaf_size", config_.filters.adaptive_voxel.leaf_size, 0.1))
  {
    ROS_DEBUG_STREAM("Did not load filters/adaptive_voxel/leaf_size. Standard value is: " << config_.filters.adaptive_voxel.leaf_size);
  }
  if (!private_nh_.param<int>("filters/adaptive_voxel/density_thr", config_.filters.adaptive_voxel.density_thr, 50))
  {
    ROS_DEBUG_STREAM("Did not load filters/adaptive_voxel/density_thr. Standard value is: " << config_.filters.adaptive_voxel.density_thr);
  }
  // 距离自适应体素滤波参数
  if (!private_nh_.param<bool>("filters/distance_adaptive_voxel/enable", config_.filters.distance_adaptive_voxel.enable, false))
  {
    ROS_DEBUG_STREAM("Did not load filters/distance_adaptive_voxel/enable. Standard value is: " << config_.filters.distance_adaptive_voxel.enable);
  }
  ...
  {
    ROS_DEBUG_STREAM("Did not load filters/distance_adaptive_voxel/near_leaf. Standard value is: " << config_.filters.distance_adaptive_voxel.near_leaf);
  }
  ...
  {
    ROS_DEBUG_STREAM("Did not load filters/distance_adaptive_voxel/far_leaf. Standard value is: " << config_.filters.distance_adaptive_voxel.far_leaf);
  }
  ...
  {
    ROS_DEBUG_STREAM("Did not load filters/distance_adaptive_voxel/dist_threshold. Standard value is: " << config_.filters.distance_adaptive_voxel.dist_threshold);
  }
  // 强度滤波参数
  if (!private_nh_.param<bool>("filters/intensity/enable", config_.filters.intensity.enable, false))
  {
    ROS_DEBUG_STREAM("Did not load filters/intensity/enable. Standard value is: " << config_.filters.intensity.enable);
  }
  {
    ...
    {
      ROS_DEBUG_STREAM("Did not load filters/intensity/min_intensity. Standard value is: " << min_intensity_d);
    }
  ...
  {
    ROS_DEBUG_STREAM("Did not load filters/use_cropbox. Standard value is: " << config_.filters.use_cropbox);
  }
  ...
  {
    ROS_DEBUG_STREAM("Did not load patchworkpp/enable_rnr. Standard value is: " << config_.patchworkpp.enable_rnr);
  }
  if (!private_nh_.param<bool>("patchworkpp/enable_rvpf", config_.patchworkpp.enable_rvpf, true))
  {
    ROS_DEBUG_STREAM("Did not load patchworkpp/enable_rvpf. Standard value is: " << config_.patchworkpp.enable_rvpf);
  }
  if (!private_nh_.param<bool>("patchworkpp/enable_tgr", config_.patchworkpp.enable_tgr, true))
  {
    ROS_DEBUG_STREAM("Did not load patchworkpp/enable_tgr. Standard value is: " << config_.patchworkpp.enable_tgr);
  }
  if (!private_nh_.param<int>("patchworkpp/num_iter", config_.patchworkpp.num_iter, 3))
  {
    ROS_DEBUG_STREAM("Did not load patchworkpp/num_iter. Standard value is: " << config_.patchworkpp.num_iter);
  }
  if (!private_nh_.param<int>("patchworkpp/num_lpr", config_.patchworkpp.num_lpr, 20))
  {
    ROS_DEBUG_STREAM("Did not load patchworkpp/num_lpr. Standard value is: " << config_.patchworkpp.num_lpr);
  }
  if (!private_nh_.param<int>("patchworkpp/num_min_pts", config_.patchworkpp.num_min_pts, 10))
  {
    ROS_DEBUG_STREAM("Did not load patchworkpp/num_min_pts. Standard value is: " << config_.patchworkpp.num_min_pts);
  }
  if (!private_nh_.param<int>("patchworkpp/num_zones", config_.patchworkpp.num_zones, 4))
  {
    ROS_DEBUG_STREAM("Did not load patchworkpp/num_zones. Standard value is: " << config_.patchworkpp.num_zones);
  }
  if (!private_nh_.param<int>("patchworkpp/num_rings_of_interest", config_.patchworkpp.num_rings_of_interest, 4))
  {
    ROS_DEBUG_STREAM("Did not load patchworkpp/num_rings_of_interest. Standard value is: " << config_.patchworkpp.num_rings_of_interest);
  }
  if (!private_nh_.param<double>("patchworkpp/rnr_ver_angle_thr", config_.patchworkpp.rnr_ver_angle_thr, -15.0))
  {
    ROS_DEBUG_STREAM("Did not load patchworkpp/rnr_ver_angle_thr. Standard value is: " << config_.patchworkpp.rnr_ver_angle_thr);
  }
  if (!private_nh_.param<double>("patchworkpp/rnr_intensity_thr", config_.patchworkpp.rnr_intensity_thr, 0.2))
  {
    ROS_DEBUG_STREAM("Did not load patchworkpp/rnr_intensity_thr. Standard value is: " << config_.patchworkpp.rnr_intensity_thr);
  }
  if (!private_nh_.param<double>("patchworkpp/th_seeds", config_.patchworkpp.th_seeds, 0.125))
  {
    ROS_DEBUG_STREAM("Did not load patchworkpp/th_seeds. Standard value is: " << config_.patchworkpp.th_seeds);
  }
  if (!private_nh_.param<double>("patchworkpp/th_dist", config_.patchworkpp.th_dist, 0.125))
  {
    ROS_DEBUG_STREAM("Did not load patchworkpp/th_dist. Standard value is: " << config_.patchworkpp.th_dist);
  }
  if (!private_nh_.param<double>("patchworkpp/th_seeds_v", config_.patchworkpp.th_seeds_v, 0.25))
  {
    ROS_DEBUG_STREAM("Did not load patchworkpp/th_seeds_v. Standard value is: " << config_.patchworkpp.th_seeds_v);
  }
  if (!private_nh_.param<double>("patchworkpp/th_dist_v", config_.patchworkpp.th_dist_v, 0.1))
  {
    ROS_DEBUG_STREAM("Did not load patchworkpp/th_dist_v. Standard value is: " << config_.patchworkpp.th_dist_v);
  }
  if (!private_nh_.param<double>("patchworkpp/max_range", config_.patchworkpp.max_range, 80.0))
  {
    ROS_DEBUG_STREAM("Did not load patchworkpp/max_range. Standard value is: " << config_.patchworkpp.max_range);
  }
  if (!private_nh_.param<double>("patchworkpp/min_range", config_.patchworkpp.min_range, 2.7))
  {
    ROS_DEBUG_STREAM("Did not load patchworkpp/min_range. Standard value is: " << config_.patchworkpp.min_range);
  }
  if (!private_nh_.param<double>("patchworkpp/uprightness_thr", config_.patchworkpp.uprightness_thr, 0.707))
  {
    ROS_DEBUG_STREAM("Did not load patchworkpp/uprightness_thr. Standard value is: " << config_.patchworkpp.uprightness_thr);
  }
  ...
  {
    ROS_DEBUG_STREAM("Did not load patchworkpp/adaptive_seed_selection_margin. Standard value is: " << config_.patchworkpp.adaptive_seed_selection_margin);
  }
  if (!private_nh_.param<int>("patchworkpp/max_flatness_storage", config_.patchworkpp.max_flatness_storage, 1000))
  {
    ROS_DEBUG_STREAM("Did not load patchworkpp/max_flatness_storage. Standard value is: " << config_.patchworkpp.max_flatness_storage);
  }
  if (!private_nh_.param<int>("patchworkpp/max_elevation_storage", config_.patchworkpp.max_elevation_storage, 1000))
  {
    ROS_DEBUG_STREAM("Did not load patchworkpp/max_elevation_storage. Standard value is: " << config_.patchworkpp.max_elevation_storage);
  }
  ...
  {
    ROS_DEBUG_STREAM("Did not load patchworkpp/th_dist_far_scale. Standard value is: " << config_.patchworkpp.th_dist_far_scale);
  }
  if (!private_nh_.param<double>("patchworkpp/min_normal_z", config_.patchworkpp.min_normal_z, 0.7))
  {
    ROS_DEBUG_STREAM("Did not load patchworkpp/min_normal_z. Standard value is: " << config_.patchworkpp.min_normal_z);
  }
  if (!private_nh_.param<double>("patchworkpp/far_zone_min_pts_scale", config_.patchworkpp.far_zone_min_pts_scale, 2.0))
  {
    ROS_DEBUG_STREAM("Did not load patchworkpp/far_zone_min_pts_scale. Standard value is: " << config_.patchworkpp.far_zone_min_pts_scale);
  }
  ...
  {
    ROS_DEBUG_STREAM("Did not load str_seg_distance. Standard value is: " << config_.str_seg_distance);
  }
  ...
  {
    ROS_DEBUG_STREAM("Did not load cluster/adaptive_size/enable. Standard value is: " << config_.cluster.adaptive_size.enable);
  }
  if (!private_nh_.param<int>("cluster/adaptive_size/near_min_size", config_.cluster.adaptive_size.near_min_size, 3))
  {
    ROS_DEBUG_STREAM("Did not load cluster/adaptive_size/near_min_size. Standard value is: " << config_.cluster.adaptive_size.near_min_size);
  }
  if (!private_nh_.param<int>("cluster/adaptive_size/near_max_size", config_.cluster.adaptive_size.near_max_size, 100))
  {
    ROS_DEBUG_STREAM("Did not load cluster/adaptive_size/near_max_size. Standard value is: " << config_.cluster.adaptive_size.near_max_size);
  }
  if (!private_nh_.param<int>("cluster/adaptive_size/far_min_size", config_.cluster.adaptive_size.far_min_size, 1))
  {
    ROS_DEBUG_STREAM("Did not load cluster/adaptive_size/far_min_size. Standard value is: " << config_.cluster.adaptive_size.far_min_size);
  }
  if (!private_nh_.param<int>("cluster/adaptive_size/far_max_size", config_.cluster.adaptive_size.far_max_size, 30))
  {
    ROS_DEBUG_STREAM("Did not load cluster/adaptive_size/far_max_size. Standard value is: " << config_.cluster.adaptive_size.far_max_size);
  }
  if (!private_nh_.param<int>("cluster/min_cluster_size", config_.cluster.min_cluster_size, 1))
  {
    ROS_DEBUG_STREAM("Did not load cluster/min_cluster_size. Standard value is: " << config_.cluster.min_cluster_size);
  }
  if (!private_nh_.param<int>("cluster/max_cluster_size", config_.cluster.max_cluster_size, 50))
  {
    ROS_DEBUG_STREAM("Did not load cluster/max_cluster_size. Standard value is: " << config_.cluster.max_cluster_size);
  }
  // 聚类方法选择
  ...
  {
    ROS_DEBUG_STREAM("Did not load cluster/dbscan/eps. Standard value is: " << config_.cluster.dbscan.eps);
  }
  if (!private_nh_.param<int>("cluster/dbscan/min_pts", config_.cluster.dbscan.min_pts, 3))
  {
    ROS_DEBUG_STREAM("Did not load cluster/dbscan/min_pts. Standard value is: " << config_.cluster.dbscan.min_pts);
  }
  if (!private_nh_.param<bool>("cluster/dbscan/adaptive_eps", config_.cluster.dbscan.adaptive_eps, true))
  {
    ROS_DEBUG_STREAM("Did not load cluster/dbscan/adaptive_eps. Standard value is: " << config_.cluster.dbscan.adaptive_eps);
  }
  if (!private_nh_.param<double>("cluster/dbscan/eps_near", config_.cluster.dbscan.eps_near, 0.15))
  {
    ROS_DEBUG_STREAM("Did not load cluster/dbscan/eps_near. Standard value is: " << config_.cluster.dbscan.eps_near);
  }
  if (!private_nh_.param<double>("cluster/dbscan/eps_far", config_.cluster.dbscan.eps_far, 0.5))
  {
    ROS_DEBUG_STREAM("Did not load cluster/dbscan/eps_far. Standard value is: " << config_.cluster.dbscan.eps_far);
  }
  // VLP-16 参数
  if (!private_nh_.param<double>("cluster/vlp16/cluster_tolerance", config_.cluster.vlp16.cluster_tolerance, 0.3))
  {
    ROS_DEBUG_STREAM("Did not load cluster/vlp16/cluster_tolerance. Standard value is: " << config_.cluster.vlp16.cluster_tolerance);
  }
  if (!private_nh_.param<double>("cluster/vlp16/max_bbox_x", config_.cluster.vlp16.max_bbox_x, 0.4))
  {
    ROS_DEBUG_STREAM("Did not load cluster/vlp16/max_bbox_x. Standard value is: " << config_.cluster.vlp16.max_bbox_x);
  }
  if (!private_nh_.param<double>("cluster/vlp16/max_bbox_y", config_.cluster.vlp16.max_bbox_y, 0.4))
  {
    ROS_DEBUG_STREAM("Did not load cluster/vlp16/max_bbox_y. Standard value is: " << config_.cluster.vlp16.max_bbox_y);
  }
  if (!private_nh_.param<double>("cluster/vlp16/max_bbox_z", config_.cluster.vlp16.max_bbox_z, 0.5))
  {
    ROS_DEBUG_STREAM("Did not load cluster/vlp16/max_bbox_z. Standard value is: " << config_.cluster.vlp16.max_bbox_z);
  }
  // point_clip 参数
  if (!private_nh_.param<double>("cluster/point_clip/min_distance", config_.cluster.point_clip.min_distance, 1.0))
  {
    ROS_DEBUG_STREAM("Did not load cluster/point_clip/min_distance. Standard value is: " << config_.cluster.point_clip.min_distance);
  }
  if (!private_nh_.param<double>("cluster/point_clip/max_distance", config_.cluster.point_clip.max_distance, 15.0))
  {
    ROS_DEBUG_STREAM("Did not load cluster/point_clip/max_distance. Standard value is: " << config_.cluster.point_clip.max_distance);
  }
  // FEC (Fast Euclidean Clustering) 参数
  if (!private_nh_.param<bool>("cluster/fec/enable", config_.cluster.fec.enable, true))
  {
    ROS_DEBUG_STREAM("Did not load cluster/fec/enable. Standard value is: " << config_.cluster.fec.enable);
  }
  if (!private_nh_.param<double>("cluster/fec/quality", config_.cluster.fec.quality, 0.3))
  {
    ROS_DEBUG_STREAM("Did not load cluster/fec/quality. Standard value is: " << config_.cluster.fec.quality);
  }
  // 多帧累积参数
  if (!private_nh_.param<bool>("cluster/multi_frame/enable", config_.cluster.multi_frame.enable, true))
  {
    ROS_DEBUG_STREAM("Did not load cluster/multi_frame/enable. Standard value is: " << config_.cluster.multi_frame.enable);
  }
  if (!private_nh_.param<int>("cluster/multi_frame/num_frames", config_.cluster.multi_frame.num_frames, 2))
  {
    ROS_DEBUG_STREAM("Did not load cluster/multi_frame/num_frames. Standard value is: " << config_.cluster.multi_frame.num_frames);
  }
  if (!private_nh_.param<double>("cluster/multi_frame/max_distance", config_.cluster.multi_frame.max_distance, 10.0))
  {
    ROS_DEBUG_STREAM("Did not load cluster/multi_frame/max_distance. Standard value is: " << config_.cluster.multi_frame.max_distance);
  }
