...
class VehicleDynamics {
 public:
  VehicleDynamics();
  explicit VehicleDynamics(const VehicleParams& vehicle_params, const TireParams& tire_params);
 ...
 private:
   ...
   */
  std::array<double, 6> stateDerivative(const VehicleState& state, const ControlInput& input);
   ...
   */
  VehicleState kinematicCorrection(const VehicleState& x_dyn, const VehicleState& x_prev,
