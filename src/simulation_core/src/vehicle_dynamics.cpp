...
VehicleDynamics::VehicleDynamics() : vehicle_params_(), tire_model_(), state_() {
  tire_Fx_.fill(0.0);
  tire_Fy_.fill(0.0);
}

VehicleDynamics::VehicleDynamics(const VehicleParams& vehicle_params, const TireParams& tire_params)
...
void VehicleDynamics::calculateNormalLoads() {
  ...
  if (state_.input.throttle > 0.01) {
    ax = state_.input.throttle * vehicle_params_.max_torque / vehicle_params_.wheel_radius / m;
  } else if (state_.input.brake > 0.01) {
    ax = -state_.input.brake * vehicle_params_.max_brake_torque / vehicle_params_.wheel_radius / m;
  }
...
void VehicleDynamics::calculateTireForces(double& Fx_total, double& Fy_total, double& Mz_total) {
  const double delta = state_.vehicle.steering;
  const double lf = vehicle_params_.cg_to_front;
  const double lr = vehicle_params_.cg_to_rear;
  const double tw = vehicle_params_.track_width / 2.0;
  ...
  Mz_total = lf * (Fy_fl + Fy_fr) - lr * (Fy_rl + Fy_rr) + tw * (Fx_fr - Fx_fl + Fx_rr - Fx_rl);
...
void VehicleDynamics::integrateRK4(const ControlInput& input, double dt) {
  ...
  while (state_.vehicle.yaw > M_PI)
    state_.vehicle.yaw -= 2 * M_PI;
  while (state_.vehicle.yaw < -M_PI)
    state_.vehicle.yaw += 2 * M_PI;
...
VehicleState VehicleDynamics::kinematicCorrection(const VehicleState& x_dyn,
                                                  ...
                                                  const ControlInput& input, double dt) {
...
void VehicleDynamics::applySteeringDynamics(double target, double dt) {
  ...
  target = std::clamp(target, -vehicle_params_.max_steering, vehicle_params_.max_steering);
