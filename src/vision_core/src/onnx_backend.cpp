...
bool OnnxBackend::initialize(const InferenceConfig& config) {
  ...
  try {
    ...
    session_ = std::make_unique<Ort::Session>(env_, config_.model_path.c_str(), opts);
    ready_ = true;
    return true;
...
cv::Mat OnnxBackend::preprocess(const cv::Mat& bgr) const {
  ...
  cv::dnn::blobFromImage(resized, blob, 1.0 / 255.0,
                         cv::Size(config_.input_width, config_.input_height), cv::Scalar(), true,
                         false, CV_32F);
  return blob;
}
...
std::vector<Detection> OnnxBackend::postprocess(const float* data, int num_proposals,
