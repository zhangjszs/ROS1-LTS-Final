...
PointCloud::Ptr generateSyntheticCloud(size_t num_points, double ground_ratio = 0.6) {
  ...
  for (size_t i = 0; i < num_points; ++i) {
    ...
    if (i < ground_points) {
      ...
      pt.z = static_cast<float>(slope * std::sqrt(pt.x * pt.x + pt.y * pt.y) + dist_ground_z(gen) -
                                0.135);
    } else {
      // 障碍物点
    ...
    for (const auto& entry : fs::directory_iterator(path)) {
      if (entry.path().extension() == ".pcd") {
        ...
        if (pcl::io::loadPCDFile(entry.path().string(), *cloud) == 0) {
          ...
          std::cout << "Loaded: " << entry.path().filename() << " (" << cloud->size()
                    << " points)\n";
        }
      }
    }
...
void printReport(const std::vector<PerfStats>& all_stats, size_t cloud_size) {
  ...
  std::cout << "--------------------------------------------------------------------------------\n";
  std::cout << std::left << std::setw(15) << "算法" << std::right << std::setw(12) << "平均(ms)"
            << std::setw(12) << "最小(ms)" << std::setw(12) << "最大(ms)" << std::setw(12)
            << "标准差(ms)" << std::setw(10) << "FPS" << std::setw(12) << "地面比例"
            << "\n";
  std::cout << "--------------------------------------------------------------------------------\n";

  for (const auto& stats : all_stats) {
    std::cout << std::left << std::setw(15) << stats.algorithm_name << std::right << std::fixed
              << std::setprecision(2) << std::setw(12) << stats.mean_time() << std::setw(12)
              << stats.min_time() << std::setw(12) << stats.max_time() << std::setw(12)
              << stats.stddev_time() << std::setw(10) << stats.throughput_fps() << std::setw(11)
              << (stats.mean_ground_ratio() * 100) << "%"
              << "\n";
  }
...
void saveCSV(const std::vector<PerfStats>& all_stats, const std::string& filename) {
  ...
  for (const auto& stats : all_stats) {
    file << stats.algorithm_name << "," << std::fixed << std::setprecision(3) << stats.mean_time()
         << "," << stats.min_time() << "," << stats.max_time() << "," << stats.stddev_time() << ","
         << stats.throughput_fps() << "," << stats.mean_ground_ratio() << "\n";
  }
