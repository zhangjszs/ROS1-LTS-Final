...
struct PolarBin {
  ...
  double getMeanZ() const { return count > 0 ? sum_z / count : 0.0; }
};
  ...
  double getSegmentHeight(double r, int bin_idx, int max_extrapolate_bins = 5) const {
    ...
    for (const auto& seg : segments) {
      if (!seg.valid)
        continue;
      int d = 0;
      if (bin_idx < seg.bin_start)
        d = seg.bin_start - bin_idx;
      else if (bin_idx >= seg.bin_end)
        d = bin_idx - seg.bin_end + 1;
      if (d < min_dist_bins) {
        min_dist_bins = d;
        nearest = &seg;
      }
...
class FastGroundSegmentation {
 ...
 private:
   ...
   * @brief 计算点到极坐标索引
   */
  inline void getGridIndex(const PointType& pt, int& sector_idx, int& bin_idx,
...
inline void FastGroundSegmentation::getGridIndex(const PointType& pt, int& sector_idx, int& bin_idx,
                                                 double& distance) const {
  // 计算极坐标
  ...
  if (sector_idx >= config_.num_sectors)
    sector_idx = config_.num_sectors - 1;
  if (sector_idx < 0)
    sector_idx = 0;

  bin_idx = static_cast<int>((distance - config_.min_range) * inv_bin_size_);
  if (bin_idx >= config_.num_bins)
    bin_idx = config_.num_bins - 1;
  if (bin_idx < 0)
    bin_idx = 0;
}
