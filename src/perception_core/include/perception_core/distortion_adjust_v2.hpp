...
struct ImuLidarExtrinsics {
  ...
  Eigen::Vector3d transformVector(const Eigen::Vector3d& v_imu) const { return rotation * v_imu; }
...
class DistortionAdjustV2 {
 public:
  ...
  bool CompensateWithTime(const typename pcl::PointCloud<PointT>::Ptr& cloud, CloudPtr& output,
 ...
 private:
   ...
   * @brief 补偿单个点（预积分模式）
   */
  void CompensatePointPreintegration(PointType& point, double point_time, double ref_time,
...
template <typename PointT>
bool DistortionAdjustV2::CompensateWithTime(const typename pcl::PointCloud<PointT>::Ptr& cloud,
                                            CloudPtr& output, double cloud_timestamp) {
  ...
  // pt.time 语义校验：检查首个点的time字段范围
  {
    ...
    if (t0 > 1e6) {
      ...
      debug_info_.error_msg =
          "pt.time appears to be absolute timestamp, expected relative offset [0, scan_period]";
      return false;
    }
  ...
  for (const auto& pt : cloud->points) {
    ...
    // 计算位移量（用于调试）
    double disp = std::sqrt(std::pow(out_pt.x - pt.x, 2) + std::pow(out_pt.y - pt.y, 2) +
