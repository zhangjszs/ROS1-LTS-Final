...
bool FastGroundSegmentation::isGroundPoint(const PointType& pt, int sector_idx, int bin_idx,
...
void FastGroundSegmentation::segment(const pcl::PointCloud<PointType>::Ptr& input,
  ...
  // 确保输出点云已分配
  if (!ground)
    ground = pcl::PointCloud<PointType>::Ptr(new pcl::PointCloud<PointType>);
  if (!non_ground)
...
void FastGroundSegmentation::temporalSmooth() {
  ...
  if (has_prev_models_ && static_cast<int>(prev_sector_models_.size()) == config_.num_sectors) {
