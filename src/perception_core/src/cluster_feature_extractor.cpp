...
void ClusterFeatureExtractor::computeGeometricFeatures(
    const pcl::PointCloud<PointType>::Ptr& cluster, ClusterFeatures& features) {
...
void ClusterFeatureExtractor::computeShapeFeatures(const pcl::PointCloud<PointType>::Ptr& cluster,
                                                   ClusterFeatures& features) {
  ...
  // PCL getEigenValues() 返回降序: λ₀ ≥ λ₁ ≥ λ₂
  // λ₀ = 最大 (主方向), λ₂ = 最小 (厚度方向)
  if (eigenvalues[2] > 1e-6) {
    features.shape_elongation = eigenvalues[0] / eigenvalues[2];  // λ_max / λ_min
    features.shape_planarity =
        (eigenvalues[1] - eigenvalues[2]) / eigenvalues[0];  // (λ_mid - λ_min) / λ_max
    features.linearity =
        (eigenvalues[0] - eigenvalues[1]) / eigenvalues[0];  // (λ_max - λ_mid) / λ_max
  }
