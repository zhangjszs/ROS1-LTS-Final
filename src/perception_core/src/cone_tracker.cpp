...
std::vector<std::pair<int, int>> ConeTracker::hungarianAssign(
    const std::vector<std::vector<double>>& cost_matrix, int n_rows, int n_cols, double threshold) {
  ...
  for (int j = 1; j <= n; ++j) {
    int i = p[j] - 1;
    int jj = j - 1;
    if (i >= 0 && i < n_rows && jj < n_cols && cost_matrix[i][jj] <= threshold) {
...
void ConeTracker::updateTrack(TrackedCone& track, const Detection& det) {
  double alpha = config_.measurement_noise / (config_.measurement_noise + config_.process_noise);
