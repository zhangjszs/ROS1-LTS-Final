...
void TopologyRepair::estimateTrackDirection(const std::vector<TopologyCone>& cones, double& dir_x,
                                            double& dir_y) {
  if (cones.size() < 2) {
    dir_x = 1.0;
    dir_y = 0.0;
...
std::vector<TopologyCone> TopologyRepair::repair(const std::vector<TopologyCone>& cones) {
  ...
  auto fillGaps = [&](const std::vector<size_t>& sorted_indices) {
    for (size_t k = 1; k < sorted_indices.size(); ++k) {
      ...
      if (is_outlier[i0] || is_outlier[i1])
        continue;
