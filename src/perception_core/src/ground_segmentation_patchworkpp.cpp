...
void lidar_cluster::ground_segmentation_dispatch_(
  ...
  if (ground_method_ != "ransac") {
    std::cerr << "Unknown ground_method: " << ground_method_
              << ", valid: ransac|patchworkpp|fgs, fallback to ransac" << std::endl;
  }
  ground_segmentation_ransac_(in_pc, g_not_ground_pc);
}

void lidar_cluster::ground_segmentation_patchworkpp_(
    const pcl::PointCloud<PointType>::Ptr& in_pc,
    pcl::PointCloud<PointType>::Ptr& g_not_ground_pc) {
