...
VoxelKey ComputeVoxelKey(const PointType& p, double leaf_size) {
  return VoxelKey{static_cast<int>(std::floor(p.x / leaf_size)),
...
void AdaptiveVoxelGrid(const pcl::PointCloud<PointType>::Ptr& in,
                       pcl::PointCloud<PointType>::Ptr& out, double leaf_size, int density_thr) {
...
void DistanceAdaptiveVoxelFilter(const pcl::PointCloud<PointType>::Ptr& in,
                                 ...
                                 double dist_threshold)  // 距离阈值
{
  ...
  if (near_leaf > 0 && !near_cloud->empty()) {
    voxel.setLeafSize(static_cast<float>(near_leaf), static_cast<float>(near_leaf),
  ...
  if (far_leaf > 0 && !far_cloud->empty()) {
    voxel.setLeafSize(static_cast<float>(far_leaf), static_cast<float>(far_leaf),
...
// 强度滤波 - 去除低强度噪声点
void IntensityFilter(pcl::PointCloud<PointType>::Ptr& cloud, float min_intensity) {
...
float euc_distance(PointType a) {
...
void point_clip(const pcl::PointCloud<PointType>::Ptr in, double min_dist, double max_dist) {
  ...
  for (size_t i = 0; i < in->points.size(); i++) {
    ...
    if (tmp_euc <= max_dist && in->points[i].x > 0 && tmp_euc >= min_dist) {
...
void lidar_cluster::Configure(const LidarClusterConfig& config) {
  ...
  roi_mode_ = roi_.mode;
  std::transform(roi_mode_.begin(), roi_mode_.end(), roi_mode_.begin(),
  ...
  scorer_config.track_semantic.expected_track_width =
      config.confidence.track_semantic.expected_track_width;
  scorer_config.track_semantic.expected_cone_spacing =
      config.confidence.track_semantic.expected_cone_spacing;
  scorer_config.track_semantic.spacing_tolerance =
      config.confidence.track_semantic.spacing_tolerance;
  ...
  scorer_config.track_semantic.min_neighbors_hard =
      config.confidence.track_semantic.min_neighbors_hard;
  scorer_config.track_semantic.max_isolation_distance =
      config.confidence.track_semantic.max_isolation_distance;
  ...
  std::transform(ground_method_.begin(), ground_method_.end(), ground_method_.begin(),
                 [](unsigned char c) { return static_cast<char>(std::tolower(c)); });
  ...
  patchwork_params_.adaptive_seed_selection_margin =
      config.patchworkpp.adaptive_seed_selection_margin;
...
// 直通滤波
void lidar_cluster::PassThrough(pcl::PointCloud<PointType>::Ptr& cloud_filtered) {
  ...
  if (road_type_ == 1) {
    ...
    if (roi_use_point_clip_) {
      ...
      point_clip(cloud_filtered, cluster_.point_clip.min_distance,
  ...
  if (filters_.use_cropbox) {
    ...
    crop.setMin(Eigen::Vector4f(static_cast<float>(x_min), static_cast<float>(y_min),
                                static_cast<float>(z_down_), 1.0f));
    crop.setMax(Eigen::Vector4f(static_cast<float>(x_max), static_cast<float>(y_max),
...
voxel_filter:
  ...
  if (filters_.distance_adaptive_voxel.enable) {
    // 距离自适应体素滤波：近处大体素，远处小体素
    pcl::PointCloud<PointType>::Ptr filtered(new pcl::PointCloud<PointType>());
    DistanceAdaptiveVoxelFilter(
        cloud_filtered, filtered, filters_.distance_adaptive_voxel.near_leaf,
        filters_.distance_adaptive_voxel.far_leaf, filters_.distance_adaptive_voxel.dist_threshold);
    ...
    AdaptiveVoxelGrid(cloud_filtered, adaptive_filtered, filters_.adaptive_voxel.leaf_size,
...
void lidar_cluster::PostGroundFilter(pcl::PointCloud<PointType>::Ptr& cloud,
  ...
  if (filters_.obstacle_height.enable) {
    ...
    const float min_dist_sq = static_cast<float>(filters_.obstacle_height.min_distance *
                                                 filters_.obstacle_height.min_distance);
    ...
    for (const auto& p : cloud->points) {
      ...
      if (cell.count < min_pts || (cell.z_max - cell.z_min) <= max_span) {
        filtered->push_back(p);
      }
    }
    filtered->width = filtered->points.size();
  ...
  if (filters_.distance_adaptive_voxel.enable) {
    pcl::PointCloud<PointType>::Ptr filtered(new pcl::PointCloud<PointType>());
    DistanceAdaptiveVoxelFilter(cloud, filtered, filters_.distance_adaptive_voxel.near_leaf,
                                filters_.distance_adaptive_voxel.far_leaf,
                                filters_.distance_adaptive_voxel.dist_threshold);
    cloud.swap(filtered);
  } else if (filters_.adaptive_voxel.enable) {
    pcl::PointCloud<PointType>::Ptr adaptive_filtered(new pcl::PointCloud<PointType>());
    AdaptiveVoxelGrid(cloud, adaptive_filtered, filters_.adaptive_voxel.leaf_size,
                      filters_.adaptive_voxel.density_thr);
...
// 带自适应聚类大小的欧式聚类
void lidar_cluster::EucClusterMethodAdaptive(pcl::PointCloud<PointType>::Ptr inputcloud,
                                             std::vector<pcl::PointIndices>& cluster_indices,
                                             const double& max_cluster_dis, int min_cluster_size,
...
// 计算自适应聚类大小（根据距离线性插值）
void lidar_cluster::getAdaptiveClusterSize(double distance, int& min_size, int& max_size) const {
...
void lidar_cluster::EuclideanAdaptiveClusterMethod(
  ...
  for (size_t i = 0; i < cloud_segments_array.size(); i++) {
    if (!cloud_segments_array[i]->points.empty()) {
      ...
      // 使用自适应参数进行聚类
      EucClusterMethodAdaptive(cloud_segments_array[i], cluster_indices[i], seg_distances[i],
                               min_size, max_size);
    }
  }
}
...
// DBSCAN 聚类实现
void lidar_cluster::DbscanClusterMethod(pcl::PointCloud<PointType>::Ptr inputcloud,
                                        std::vector<pcl::PointIndices>& cluster_indices, double eps,
                                        int min_pts, int max_cluster_size) {
  if (!inputcloud || inputcloud->points.empty()) {
    return;
  }
...
void lidar_cluster::DbscanAdaptiveClusterMethod(
  ...
  for (size_t i = 0; i < cloud_segments_array.size(); i++) {
    if (!cloud_segments_array[i]->points.empty()) {
      ...
      // DBSCAN 聚类
      DbscanClusterMethod(cloud_segments_array[i], cluster_indices[i], eps, cluster_.dbscan.min_pts,
                          max_size);
    }
  }
}
...
void lidar_cluster::clusterMethod32(LidarClusterOutput* output) {
  ...
  for (size_t i = 0; i < cluster_indices.size(); ++i) {
    for (const auto& indices : cluster_indices[i]) {
      ...
      for (int idx : indices.indices) {
        cloud_cluster->points.push_back(cloud_segments_array[i]->points[idx]);
      }
      ...
      float euc = std::sqrt(centroid[0] * centroid[0] + centroid[1] * centroid[1]);
      ...
      if (config_.roi.center_exclusion.enable) {
        ...
        if (euc >= ce.start_distance && std::fabs(centroid[1]) <= ce.y_half) {
          roi_rejected = true;
        }
      }
      ...
      if (is_cone) {
        ...
        output->cones_cloud->points.insert(output->cones_cloud->points.end(),
  ...
  if (config_.confidence.track_semantic.enable && config_.confidence.track_semantic.weight > 0.0 &&
      output->cones.size() >= 2) {
    ...
    for (auto& det : output->cones) {
      ...
      if (euc <= cc.confidence_ramp_start) {
        ...
        double t =
            (euc - cc.confidence_ramp_start) / (cc.confidence_ramp_end - cc.confidence_ramp_start);
        min_conf = cc.min_confidence_near + t * (cc.min_confidence_far - cc.min_confidence_near);
      }

      if (det.confidence > min_conf) {
        if (det.cluster) {
          new_cloud->points.insert(new_cloud->points.end(), det.cluster->points.begin(),
...
void lidar_cluster::clusterMethod16(LidarClusterOutput* output) {
  ...
  for (const auto& indices : cluster_indices) {
    ...
    bool is_cone = (xx < cluster_.vlp16.max_bbox_x && yy < cluster_.vlp16.max_bbox_y &&
...
void lidar_cluster::AccumulateFrames(pcl::PointCloud<PointType>::Ptr& cloud) {
  ...
  if (frame_buffer_.size() > 1) {
    ...
    for (size_t i = 0; i < frame_buffer_.size() - 1; ++i) {
      cloud->points.insert(cloud->points.end(), frame_buffer_[i]->points.begin(),
...
void lidar_cluster::deduplicateDetections(std::vector<ConeDetection>& cones,
  ...
  std::sort(order.begin(), order.end(),
            [&](size_t a, size_t b) { return cones[a].confidence > cones[b].confidence; });
  ...
  for (size_t i = 0; i < n; ++i) {
    ...
    for (size_t j = i + 1; j < n; ++j) {
      ...
      // Stacked cone detection: Z-axis check for vertically stacked cones
        ...
        double stacked_xy_sq =
            config_.dedup.stacked_xy_threshold * config_.dedup.stacked_xy_threshold;
        if (xy_dist_sq < stacked_xy_sq) {
          ...
          if (dz < config_.dedup.z_height_threshold) {
            ...
            if (std::abs(frac) < 0.3 &&
                static_cast<int>(std::round(ratio)) <= config_.dedup.max_layers) {
              suppressed[jdx] = true;
            }
          }
        }
  ...
  for (size_t i = 0; i < n; ++i) {
    ...
    if (kept.back().cluster) {
      new_cloud->points.insert(new_cloud->points.end(), kept.back().cluster->points.begin(),
