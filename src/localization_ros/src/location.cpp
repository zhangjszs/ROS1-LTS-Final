...
LocationNode::LocationNode(ros::NodeHandle& nh) : nh_(nh), pnh_("~"), mapper_(params_) {
  loadParameters();
  ...
  if (use_external_carstate_) {
    carstate_sub_ = nh_.subscribe<autodrive_msgs::HUAT_CarState>(
        carstate_in_topic_, 10, &LocationNode::carstateCallback, this);
  } else {
    ins_sub_ =
        nh_.subscribe<autodrive_msgs::HUAT_InsP2>(ins_topic_, 10, &LocationNode::imuCallback, this);
  }

  cone_sub_ = nh_.subscribe<autodrive_msgs::HUAT_ConeDetections>(cone_topic_, 10,
  ...
  if (debug_topics_cfg_.enabled) {
    debug_inlier_map_pub_ =
        nh_.advertise<autodrive_msgs::HUAT_ConeMap>(debug_topics_cfg_.inlier_map_topic, 10);
    debug_outlier_map_pub_ =
        nh_.advertise<autodrive_msgs::HUAT_ConeMap>(debug_topics_cfg_.outlier_map_topic, 10);
    debug_local_map_pub_ =
        nh_.advertise<autodrive_msgs::HUAT_ConeMap>(debug_topics_cfg_.local_map_topic, 10);
    debug_match_pairs_pub_ =
        nh_.advertise<std_msgs::String>(debug_topics_cfg_.match_pairs_topic, 10);
    debug_relocalization_pub_ =
        nh_.advertise<std_msgs::String>(debug_topics_cfg_.relocalization_topic, 10);
  }

  // Status diagnostics publisher
  LoadParam(pnh_, nh_, "topics/fg_status", status_topic_,
            std::string(autodrive_msgs::topic_contract::kLocalizationFgStatus));
  status_pub_ = nh_.advertise<std_msgs::String>(status_topic_, 10);
  {
    std::string diag_topic, global_diag_topic;
    bool pub_global = true;
    LoadParam(pnh_, nh_, "diagnostics_topic", diag_topic,
              std::string(autodrive_msgs::topic_contract::kLocalizationDiagnostics));
    LoadParam(pnh_, nh_, "publish_global_diagnostics", pub_global, true);
    LoadParam(pnh_, nh_, "global_diagnostics_topic", global_diag_topic,
  ...
  if (map_persistence_cfg_.enabled) {
    save_map_srv_ =
        nh_.advertiseService(map_persistence_cfg_.save_service, &LocationNode::handleSaveMap, this);
    load_map_srv_ =
        nh_.advertiseService(map_persistence_cfg_.load_service, &LocationNode::handleLoadMap, this);
  }
...
void LocationNode::loadParameters() {
  if (!LoadParam(pnh_, nh_, "length/lidarToIMUDist", params_.lidar_to_imu_dist, 1.87)) {
    ROS_WARN_STREAM(
        "Did not load lidarToIMUDist. Standard value is: " << params_.lidar_to_imu_dist);
  }
  if (!LoadParam(pnh_, nh_, "length/frontToIMUdistanceX", params_.front_to_imu_x, 0.0)) {
    ROS_WARN_STREAM(
        "Did not load frontToIMUdistanceX. Standard value is: " << params_.front_to_imu_x);
  }
  if (!LoadParam(pnh_, nh_, "length/frontToIMUdistanceY", params_.front_to_imu_y, 0.0)) {
    ROS_WARN_STREAM("Did not load frontToIMUdistanceY. Standard value is "
                    << params_.front_to_imu_y);
  }
  if (!LoadParam(pnh_, nh_, "length/frontToIMUdistanceZ", params_.front_to_imu_z, 0.0)) {
    ROS_WARN_STREAM(
        "Did not load frontToIMUdistanceZ. Standard value is: " << params_.front_to_imu_z);
  }
  ...
  if (!LoadParam(pnh_, nh_, "topics/cone", cone_topic_,
  ...
  if (!LoadParam(pnh_, nh_, "topics/global_map", global_map_topic_,
                 std::string("localization/global_map"))) {
  ...
  if (!LoadParam(pnh_, nh_, "use_external_carstate", use_external_carstate_, false)) {
    ROS_WARN_STREAM("Did not load use_external_carstate. Standard value is: "
                    << (use_external_carstate_ ? "true" : "false"));
  }
  ...
  // 缺锥补偿参数
  LoadParam(pnh_, nh_, "missing_cone_fallback/enabled", params_.missing_cone_fallback.enabled,
            false);
  LoadParam(pnh_, nh_, "missing_cone_fallback/max_interpolation_distance",
            params_.missing_cone_fallback.max_interpolation_distance, 8.0);
  LoadParam(pnh_, nh_, "missing_cone_fallback/expected_spacing",
            params_.missing_cone_fallback.expected_spacing, 5.0);
  LoadParam(pnh_, nh_, "missing_cone_fallback/min_confidence_for_interpolation",
  ...
  // 短路径抑制参数
  LoadParam(pnh_, nh_, "short_path_suppression/enabled", params_.short_path_suppression.enabled,
            false);
  LoadParam(pnh_, nh_, "short_path_suppression/min_path_length",
  ...
  LoadParam(pnh_, nh_, "short_path_suppression/reject_single_cone_paths",
  ...
  LoadParam(pnh_, nh_, "map/freeze/freeze_after_frames", params_.map_freeze.freeze_after_frames,
            300);
  LoadParam(pnh_, nh_, "map/freeze/freeze_after_cones", params_.map_freeze.freeze_after_cones, 120);
  LoadParam(pnh_, nh_, "map/freeze/allow_merge_when_frozen",
            params_.map_freeze.allow_merge_when_frozen, true);
  LoadParam(pnh_, nh_, "map/freeze/allow_new_cones_when_frozen",
            params_.map_freeze.allow_new_cones_when_frozen, false);
  ...
  if (LoadParam(pnh_, nh_, preset_prefix + "min_confidence_to_add", tmp_d,
                params_.min_confidence_to_add))
    params_.min_confidence_to_add = tmp_d;
  if (LoadParam(pnh_, nh_, preset_prefix + "min_confidence_to_merge", tmp_d,
                params_.min_confidence_to_merge))
  ...
  if (LoadParam(pnh_, nh_, preset_prefix + "expected_cone_spacing", tmp_d,
                params_.expected_cone_spacing))
  ...
  if (LoadParam(pnh_, nh_, preset_prefix + "enable_circle_validation", tmp_b,
                params_.enable_circle_validation))
  ...
  mapper_recovery_cfg_.fail_frames_to_degraded =
      std::max(1, mapper_recovery_cfg_.fail_frames_to_degraded);
  mapper_recovery_cfg_.fail_frames_to_ins_only =
      std::max(mapper_recovery_cfg_.fail_frames_to_degraded + 1,
               mapper_recovery_cfg_.fail_frames_to_ins_only);
  mapper_recovery_cfg_.success_frames_to_tracking =
      std::max(1, mapper_recovery_cfg_.success_frames_to_tracking);
  mapper_recovery_cfg_.recovery_cooldown_frames =
      std::max(0, mapper_recovery_cfg_.recovery_cooldown_frames);
  mapper_recovery_cfg_.recovery_probe_interval_frames =
      std::max(1, mapper_recovery_cfg_.recovery_probe_interval_frames);
  mapper_recovery_cfg_.min_local_cones_for_tracking =
      std::max(1, mapper_recovery_cfg_.min_local_cones_for_tracking);
  ...
  heading_sanity_cfg_.max_rate_deg_per_sec =
      std::max(0.0, heading_sanity_cfg_.max_rate_deg_per_sec);
  ...
  LoadParam(pnh_, nh_, "map_drift/max_mean_match_distance",
            drift_monitor_cfg_.max_mean_match_distance, 2.0);
  LoadParam(pnh_, nh_, "map_drift/min_local_cones", drift_monitor_cfg_.min_local_cones, 2);
  LoadParam(pnh_, nh_, "map_drift/bad_frames_to_trigger", drift_monitor_cfg_.bad_frames_to_trigger,
            15);
  LoadParam(pnh_, nh_, "map_drift/cooldown_frames", drift_monitor_cfg_.cooldown_frames, 50);
  LoadParam(pnh_, nh_, "map_drift/trigger_only_when_frozen",
            drift_monitor_cfg_.trigger_only_when_frozen, true);
  LoadParam(pnh_, nh_, "map_drift/action", drift_monitor_cfg_.action, std::string("rollback"));
  drift_monitor_cfg_.min_match_ratio =
      std::max(0.0, std::min(1.0, drift_monitor_cfg_.min_match_ratio));
  drift_monitor_cfg_.max_mean_match_distance =
      std::max(0.0, drift_monitor_cfg_.max_mean_match_distance);
  ...
  if (drift_monitor_cfg_.action != "rollback" && drift_monitor_cfg_.action != "reset" &&
  ...
  if (backend_ == "factor_graph") {
    ...
    LoadParam(pnh_, nh_, "fg/max_cone_factors_per_keyframe",
              fg_config_.max_cone_factors_per_keyframe, 10);
    ...
    LoadParam(pnh_, nh_, "fg/circle_center1_y", fg_config_.circle_center1_y,
              -params_.circle_center_dist / 2.0);
    LoadParam(pnh_, nh_, "fg/circle_center2_x", fg_config_.circle_center2_x, 0.0);
    LoadParam(pnh_, nh_, "fg/circle_center2_y", fg_config_.circle_center2_y,
              params_.circle_center_dist / 2.0);
    ...
    LoadParam(pnh_, nh_, "fg/anomaly/match_lost_duration", fg_config_.anomaly.match_lost_duration,
              2.0);
    LoadParam(pnh_, nh_, "fg/anomaly/no_cone_frames_max", fg_config_.anomaly.no_cone_frames_max,
              30);
...
void LocationNode::updateMapperStateMachine(bool frame_good) {
  ...
  if (prev_state != mapper_state_) {
    ...
    if (mapper_state_ == MapperRuntimeState::DEGRADED)
      ++mapper_degraded_entry_count_;
    if (mapper_state_ == MapperRuntimeState::INS_ONLY)
      ++mapper_ins_only_entry_count_;
    ...
    ROS_WARN_STREAM("[location] mapper_state transition "
                    ...
                    << mapperStateName(static_cast<int>(mapper_state_)) << " (frame_good="
                    << (frame_good ? "true" : "false") << ", fail=" << mapper_consecutive_failures_
                    << ", succ=" << mapper_consecutive_successes_ << ")");
  }
  // B20: Accumulate total frames in degraded/ins_only states
  if (mapper_state_ == MapperRuntimeState::DEGRADED)
    ++mapper_degraded_total_frames_;
  if (mapper_state_ == MapperRuntimeState::INS_ONLY)
    ++mapper_ins_only_total_frames_;
}
...
void LocationNode::updateHeadingSanity(const autodrive_msgs::HUAT_InsP2& msg,
  ...
  if (has_last_heading_ && stamp > last_heading_stamp_) {
    ...
    if (dt > 1e-3) {
      ...
      if (step_deg > heading_sanity_cfg_.max_step_deg) {
        ...
        ROS_WARN_THROTTLE(2.0, "[location] heading sanity: step too large %.3f deg (limit %.3f)",
      ...
      if (rate_deg_s > heading_sanity_cfg_.max_rate_deg_per_sec) {
        ...
        ROS_WARN_THROTTLE(2.0, "[location] heading sanity: rate too large %.3f deg/s (limit %.3f)",
      ...
      if (gyro_residual > heading_sanity_cfg_.max_heading_gyro_residual_rad_s) {
        ...
        ROS_WARN_THROTTLE(
            2.0, "[location] heading sanity: heading/gyro mismatch %.3f rad/s (limit %.3f)",
...
void LocationNode::publishMapperDebugTopics(const localization_core::MapUpdateStats& stats,
                                            const ros::Time& stamp) {
  if (!debug_topics_cfg_.enabled) {
    return;
  }
  ...
  {
    std::ostringstream oss;
    oss << "seq=" << stats.update_seq << ",merged=" << stats.merged_count
        << ",inserted=" << stats.inserted_count << ",outlier="
        << (stats.bbox_reject_count + stats.geometry_reject_count + stats.conf_add_reject_count +
            stats.conf_merge_reject_count + stats.frozen_reject_count)
        << ",map_frozen=" << (stats.map_frozen ? "true" : "false")
        << ",mean_match_distance=" << ToFixed(stats.mean_match_distance, 3);
    const int max_pairs =
        std::min(debug_topics_cfg_.max_pairs, static_cast<int>(stats.associations.size()));
    for (int i = 0; i < max_pairs; ++i) {
      const auto& a = stats.associations[i];
      oss << ";pair" << i << "{id=" << a.matched_id << ",d=" << ToFixed(a.distance, 3)
          << ",merged=" << (a.merged ? "1" : "0") << ",obs=(" << ToFixed(a.detection_base.x, 2)
          << "," << ToFixed(a.detection_base.y, 2) << ")"
  ...
  {
    std::ostringstream oss;
    oss << "backend=" << backend_ << ",state=" << mapperStateName(static_cast<int>(mapper_state_))
        << ",candidate_score=" << ToFixed(last_cone_match_ratio_, 3) << ",mode=proxy_match_ratio";
    reloc_msg.data = oss.str();
  }
  debug_relocalization_pub_.publish(reloc_msg);
}

bool LocationNode::evaluateDriftAndRecover(const localization_core::MapUpdateStats& stats,
                                           const ros::Time& stamp) {
  ...
  const bool high_residual =
      (stats.merged_count > 0 &&
       stats.mean_match_distance > drift_monitor_cfg_.max_mean_match_distance);
  ...
  if (recovered) {
    ...
    ROS_WARN_STREAM("[location] map drift recovery triggered, action="
                    << drift_monitor_cfg_.action
...
void LocationNode::publishState(const localization_core::CarState& state, const ros::Time& stamp,
  ...
  if (tf_monitor_cfg_.max_delay_sec > 0.0 && tf_last_lag_sec_ > tf_monitor_cfg_.max_delay_sec) {
    ...
    ROS_WARN_THROTTLE(2.0, "[location] TF lag exceeds threshold: lag=%.4fs limit=%.4fs",
                      tf_last_lag_sec_, tf_monitor_cfg_.max_delay_sec);
  }
  if (tf_monitor_cfg_.max_future_sec > 0.0 && tf_last_lag_sec_ < -tf_monitor_cfg_.max_future_sec) {
    ++tf_future_stamp_count_;
    ROS_WARN_THROTTLE(2.0, "[location] TF stamp is in future: lag=%.4fs tolerance=%.4fs",
  ...
  if (has_last_state_ && tf_monitor_cfg_.max_gap_sec > 0.0) {
    ...
    if (dt > tf_monitor_cfg_.max_gap_sec) {
      ...
      ROS_WARN_THROTTLE(2.0, "[location] TF publish gap exceeds threshold: dt=%.4fs limit=%.4fs",
  ...
  if (has_last_state_) {
    ...
    if (dt > 1e-3) {
      ...
      const double dtheta =
          fsd_common::AngleDiff(state.car_state.theta, last_state_.car_state.theta);
      yaw_rate = dtheta / dt;
    }
  }
...
void LocationNode::publishEntryHealth(const std::string& source, const ros::Time& stamp,
                                      bool force) {
  ...
  kvs.push_back(
      KV::KV("mapper_consecutive_failures", std::to_string(mapper_consecutive_failures_)));
  kvs.push_back(
      KV::KV("mapper_consecutive_successes", std::to_string(mapper_consecutive_successes_)));
  kvs.push_back(KV::KV("mapper_ins_only_frames", std::to_string(mapper_ins_only_frames_)));
  // B20: Degraded mode performance statistics
  kvs.push_back(
      KV::KV("mapper_degraded_entry_count", std::to_string(mapper_degraded_entry_count_)));
  kvs.push_back(
      KV::KV("mapper_ins_only_entry_count", std::to_string(mapper_ins_only_entry_count_)));
  kvs.push_back(KV::KV("mapper_recovery_count", std::to_string(mapper_recovery_count_)));
  kvs.push_back(
      KV::KV("mapper_degraded_total_frames", std::to_string(mapper_degraded_total_frames_)));
  kvs.push_back(
      KV::KV("mapper_ins_only_total_frames", std::to_string(mapper_ins_only_total_frames_)));
  ...
  kvs.push_back(
      KV::KV("heading_range_violation_count", std::to_string(heading_range_violation_count_)));
  kvs.push_back(
      KV::KV("heading_step_violation_count", std::to_string(heading_jump_violation_count_)));
  kvs.push_back(
      KV::KV("heading_rate_violation_count", std::to_string(heading_rate_violation_count_)));
  kvs.push_back(
      KV::KV("heading_gyro_mismatch_count", std::to_string(heading_gyro_mismatch_count_)));
  ...
  kvs.push_back(
      KV::KV("cone_non_monotonic_new_id_count", std::to_string(cone_non_monotonic_new_id_count_)));
  ...
  if (has_last_map_stats_) {
    ...
    kvs.push_back(
        KV::KV("mapper_bbox_reject_count", std::to_string(last_map_stats_.bbox_reject_count)));
    kvs.push_back(KV::KV("mapper_geometry_reject_count",
                         std::to_string(last_map_stats_.geometry_reject_count)));
    kvs.push_back(KV::KV("mapper_conf_add_reject_count",
                         std::to_string(last_map_stats_.conf_add_reject_count)));
    kvs.push_back(KV::KV("mapper_conf_merge_reject_count",
                         std::to_string(last_map_stats_.conf_merge_reject_count)));
    kvs.push_back(
        KV::KV("mapper_frozen_reject_count", std::to_string(last_map_stats_.frozen_reject_count)));
    ...
    kvs.push_back(
        KV::KV("mapper_local_output_count", std::to_string(last_map_stats_.local_output_count)));
    kvs.push_back(
        KV::KV("mapper_mean_match_distance", ToFixed(last_map_stats_.mean_match_distance, 3)));
    kvs.push_back(
        KV::KV("mapper_max_match_distance", ToFixed(last_map_stats_.max_match_distance, 3)));
  }

  diag_helper_.PublishStatus("localization_entry_health", "localization_ros/location_node", level,
...
void LocationNode::imuCallback(const autodrive_msgs::HUAT_InsP2::ConstPtr& msg) {
  ...
  if (mapper_updated) {
    ...
    if (!heading_init_logged_ && mapper_.has_carstate()) {
      ROS_INFO("[localization] Heading_init (standard_azimuth) = %.4f deg",
...
void LocationNode::ToRos(const localization_core::CarState& state,
                         autodrive_msgs::HUAT_CarState* out) {
  if (!out) {
    return;
  }
...
localization_core::ConeDetections LocationNode::ToCore(
    const autodrive_msgs::HUAT_ConeDetections& msg) {
  ...
  for (size_t i = 0; i < msg.points.size(); ++i) {
    ...
    if (!std::isfinite(det.bbox_max.x))
      det.bbox_max.x = 0.0;
    if (!std::isfinite(det.bbox_max.y))
      det.bbox_max.y = 0.0;
    if (!std::isfinite(det.bbox_max.z))
      det.bbox_max.z = 0.0;
    if (!std::isfinite(det.bbox_min.x))
      det.bbox_min.x = 0.0;
    if (!std::isfinite(det.bbox_min.y))
      det.bbox_min.y = 0.0;
    if (!std::isfinite(det.bbox_min.z))
      det.bbox_min.z = 0.0;
...
bool LocationNode::feedFactorGraph(const autodrive_msgs::HUAT_InsP2& msg) {
  ...
  if (fg_optimizer_->TryUpdate(current_pose, t)) {
    ...
    ROS_INFO_THROTTLE(
        5.0,
        "[FG shadow] kf=%lu pose=(%.2f,%.2f,%.3f) opt_ms=%.1f lm=%zu chi2=%.2f match=%.2f state=%s",
        fg_optimizer_->NumKeyframes(), fg_pose.x, fg_pose.y, fg_pose.theta,
        fg_optimizer_->LastOptTimeMs(), fg_optimizer_->GetLandmarks().size(),
        fg_optimizer_->GetChi2Normalized(), fg_optimizer_->GetConeMatchRatio(), state_name);
    ...
    // Publish structured diagnostics
    {
      ...
      ds.values.push_back(
          kv("chi2_normalized", std::to_string(fg_optimizer_->GetChi2Normalized())));
      ds.values.push_back(
          kv("cone_match_ratio", std::to_string(fg_optimizer_->GetConeMatchRatio())));
      ds.values.push_back(kv("num_keyframes", std::to_string(fg_optimizer_->NumKeyframes())));
      ds.values.push_back(
          kv("num_landmarks", std::to_string(fg_optimizer_->GetLandmarks().size())));
      ...
      ds.values.push_back(kv("reloc_success_rate",
                             fg_reloc_attempt_count_ > 0
                                 ? std::to_string(static_cast<double>(fg_reloc_success_count_) /
                                                  fg_reloc_attempt_count_)
                                 : "n/a"));
      ds.values.push_back(
          kv("reloc_avg_ms", fg_reloc_attempt_count_ > 0
