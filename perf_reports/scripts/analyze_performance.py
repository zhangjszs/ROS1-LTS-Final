...
class PerformanceAnalyzer:
        ...
        for node_name, node_data in perf_data["nodes"].items():
            ...
            for metric_name, metric_data in metrics.items():
                if "p95" in metric_data and "p99" in metric_data:
                    ratio = metric_data["p99"] / (
                        metric_data["p95"] if metric_data["p95"] > 0 else 1.0
                    )
    ...
    def prioritize_issues(self, issues: List[Dict]) -> List[Dict]:
        """按影响优先级排序问题"""
        priority_map = {"critical": 1, "high": 2, "medium": 3, "warning": 3, "low": 4}
    ...
    def generate_markdown_report(self, perf_data_file: str, output_file: str = None) -> str:
        ...
        report = self._generate_report_content(
            perf_data, prioritized_issues, code_issues, metric_issues
        )
    ...
    def _generate_report_content(
                ...
                for dm in data_metrics:
                    if dm in metrics:
                        ...
                        display_value = (
                            f"{m['mean']:.0f}"
                            if dm != "bytes"
                            else f"{m['mean']:.0f} ({m['mean']/1024:.1f}KB)"
                        )
                        ...
                        lines.append(
                            f"| {dm} | {display_value} | {p95_value} | {p99_value} | {max_value} |"
                        )
                lines.append("")
        ...
        for severity in ["critical", "high", "medium", "warning", "low"]:
            ...
            for idx, issue in enumerate(severity_issues, 1):
                lines.append(
                    f"#### {idx}. {issue.get('description', issue.get('type', 'Unknown'))}\n"
                )
                ...
                if "file" in issue:
                    lines.append(
                        f"- **位置**: [{issue['file']}]({issue['file']})" f" (第 {issue['line']} 行)"
                    )
        ...
        if code_issues:
            ...
            for issue in code_issues[:5]:  # 只显示前5个
                if issue["severity"] in ["critical", "high"]:
                    lines.append(
                        f"- **[{issue['file']}]({issue['file']}:{issue['line']}**: "
                        f"{issue['description']}"
                    )
        ...
        if "nodes" in perf_data:
            for node_name, node_data in perf_data["nodes"].items():
                ...
                if "t_total_ms" in metrics:
                    ...
                    lines.append(
                        f"| {node_name} | {total['mean']:.2f}ms | {total['p99']:.2f}ms | "
