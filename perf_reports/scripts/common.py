    ...
    try:
        commit_hash = (
            subprocess.check_output(
                ["git", "rev-parse", "HEAD"], cwd=Config.PROJECT_ROOT, stderr=subprocess.DEVNULL
            )
            .decode()
            .strip()
        )
        ...
        return {"commit_hash": commit_hash, "commit_message": commit_msg, "branch": branch}
    except Exception:
        return {"commit_hash": "unknown", "commit_message": "unknown", "branch": "unknown"}
...
def get_system_info() -> Dict:
    ...
    try:
        cpu_info = subprocess.check_output(["lscpu"], stderr=subprocess.DEVNULL).decode()
    ...
    try:
        mem_info = subprocess.check_output(["free", "-h"], stderr=subprocess.DEVNULL).decode()
    ...
    try:
        os_info = (
            subprocess.check_output(["uname", "-s"], stderr=subprocess.DEVNULL).decode().strip()
        )
    except Exception:
        os_info = "unknown"
    ...
    try:
        os_version = (
            subprocess.check_output(["uname", "-r"], stderr=subprocess.DEVNULL).decode().strip()
        )
    except Exception:
        os_version = "unknown"
